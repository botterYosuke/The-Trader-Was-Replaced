...
- ~~**layout 永続化が screen window 未対応（中〜高）**~~ **【解決済み 2026-05-25, commit `9b5f2f19`】**: `build_layout` が `ScreenWindowRoot`（Strategy Editor / Startup、Node の left/top/width/height + `GlobalZIndex`）を `windows` 配列へ収集し、`apply_layout_system` / `apply_pending_layout_system` が world-space match の空振り後に screen match で `Node` geometry を復元する（world 側に `Without<ScreenWindowRoot>` を足して `&mut Visibility` クエリ競合を回避）。Startup は size・可視性を復元しない（[M9]）。Strategy Editor は size・可視性も layout 権威（[M14]）。save 方向は [M13] が回帰ガード。`m9` flow は実 screen window へ re-point 済み。**残: screen window の close-on-restore（layout に無い editor の despawn）は未対応**（world-space のみ。close-undo と併せて要検討）。z-order は `GlobalZIndex` を round-trip するが `WindowManager.max_z` への完全統合は未確認。
