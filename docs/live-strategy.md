...
## 5. Promote to Live と run 制御

### Promote フロー（Strategy Editor）

`[Promote to Live]` の前提条件（満たさなければボタンが disabled）:
...
`[Confirm]` で 1 コマンドが順に await する: **`RegisterLiveStrategy` → `SetExecutionMode(LiveAuto)`
→ `StartLiveStrategy`**（LiveAuto 反映前に Start が走るレースを防ぐため 1 本に集約）。
...
- ❌ `print` / `logging` 直叩き → `self.log.info(...)`（構造化ログ + `StrategyLogMessage` 中継に乗る）
