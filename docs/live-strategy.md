...
> **UI 入口（issue #40）**: 専用の「Promote to Live」ボタンと Safety Rails モーダルは撤去され、
> 起動入口はフッターの transport（再生）**▶** ボタンへ統合済み（Auto に切替 → `[File]▸[Open]` → ▶）。 [N5]

起動の前提条件:

1. Venue ログイン済み（`CONNECTED` / `SUBSCRIBED`）
2. ExecutionMode が `LiveAuto`（フッターの **Auto** セグメントで事前に切替済み）
3. 戦略の scenario に instruments が 1 件以上ある（起動銘柄は scenario から導出＝Replay Run と対称。複数件ではサイドバー選択が scenario 内ならそれを優先、無ければ先頭を使う）
4. 戦略ファイルがディスク保存済み（unsaved changes 無し）
5. Safety Rails が設定済み

フッター **▶** の起動経路では、呼び出し側（`main.rs` の transport task）が
**`RegisterLiveStrategy` → `StartLiveStrategy`** を順に await する。`SetExecutionMode(LiveAuto)`
は ▶ から再送しない: ExecutionMode はフッターの Auto セグメントで先に `LiveAuto` へ切替済み
（backend 権威）で、▶ の LiveAuto 分岐はそのモードでのみ発火するため、LiveAuto 反映前に Start が
走るレースは起きない。`RegisterLiveStrategy` はファイルを canonicalize + `sha256` 検証して
`strategy_id`（`strat-{sha256[:16]}`、内容ハッシュ由来で冪等）を発行し、`StartLiveStrategy` は
生パスではなくこの `strategy_id` を受け取る。
