...
### 6. なぜ proto 再生成後に `engine_pb2_grpc.py` を手で直すか
...
### 3.2 Backend: LiveVenueAdapter & 具象実装 🟡 骨組み完了 / 実 I/O 未実装
...
> - ⏳ 未実装: `live/event_bus.py` / `live/aggregator.py` / `tachibana_ws.py` / `tachibana_master.py` / `tachibana_login_flow.py` / `kabusapi_ws.py` / `kabusapi_*.py` REST 各種 / `kabusapi_login_flow.py` / `kabusapi_ratelimit.py` / `kabusapi_register.py` / 各 adapter の HTTP I/O 本体
...
### 3.3 Backend: live_runner.py ⏳ 未着手

> **進捗 (2026-05-18)**: 未着手。§3.2 venue I/O 本体（HTTP/WS）が動いた後に着手すべき。`replay_runner.py` と同じ位置付けの独立 asyncio 駆動ループ。`TradingNode` を使わない（誤発注の構造防止、計画書本文参照）。
