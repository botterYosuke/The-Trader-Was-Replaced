...
**累計テスト**: `python/tests/live` で **82 passed**（Step C 初期 +6 / Step C ハードン +3 / Step 1 live_runner +1 / Step 2 live_runner +5 / Step 2 ハードン live_runner +2 / Step 2e reducer_bridge +5）。回帰なし。

**フル実行 (Step 2 + ハードン完了時点)**: `cd python && uv run pytest -m "not slow"` で **512 passed (live のみ +1 後は 513 想定) / 3 skipped / 64 deselected / 0 failed**。`cargo test` も全 PASS（17 + 2 件、failed 0）。

**Step 2 完了 (2026-05-18)**: §3.3 の Step 1 で残した 4 項目を 5 サブステップ (2a〜2e) に分解し連続実装:
...
設計判断:
- LiveRunner の `subscribe()` で adapter には `{"trades", "depth"}` を購読要求するよう変更（Step 1 は `{"trades"}` のみ）。mock の inject gating は instrument 単位なのでテストは引き続き通る。
