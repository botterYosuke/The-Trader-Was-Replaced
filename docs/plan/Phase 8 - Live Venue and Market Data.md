...
## 📊 Phase 8 進捗スナップショット (2026-05-18 更新, §3.5 menu_bar + §3.5.1 + §3.6 完了)
...
| §3.5 Rust UI (`venue_capabilities.rs` / Footer toggle / menu_bar Venue+File→New+Open mode 依存 / §3.5.1 SetExecutionMode RPC) | ✅ 完了 (sidebar Tickers/SelectedSymbol/SubscribeMarketData は Phase 8 follow-up に繰越) | bevy-engine スキル準拠で実装 |
...
### 3.5 Rust UI: 接続フロー & 表示 ✅ menu_bar + §3.5.1 SetExecutionMode RPC 完了 / sidebar (Tickers/SelectedSymbol/SubscribeMarketData) は Phase 8 follow-up に繰越
