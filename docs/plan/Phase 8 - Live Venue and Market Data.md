...
ブランチ: `impl/8-venue-login-skeleton`（HEAD: 75f398e 以降）
...
**累計テスト**: `python/tests/live` で **70 passed**（Step C 初期 +6 / Step C ハードン +3 / Step 1 live_runner +1）。回帰なし。
...
### 3.3 Backend: live_runner.py ✅ Step 1 (最小 pipeline) 完了 / reducer 接続・多 interval は後続 Step
...
> - ⏳ Step 1 スコープ外 (後続 Step で対応): reducer / DataEngine 接続、Nautilus 型変換、`DepthUpdate` と直 `KlineUpdate` の pass-through、複数 interval、複数 venue。
