...
## 📊 Phase 8 進捗スナップショット (2026-05-18 更新, §3.5 sidebar 完了)
...
| §3.3 `live_runner.py` (Step 1) | ✅ 完了 | adapter→aggregator→bus の最小 pipeline、1 件 PASS（次分 tick で直前 bar 確定 emit、close は同分内 last-write-wins） |
| §3.3 `live_runner.py` (Step 2) | ✅ 完了 | DepthUpdate / venue 直送 KlineUpdate pass-through、複数 instrument、`intervals_ns=[...]` 多 interval 対応。5 件追加 PASS |
| §3.3 `live_runner.py` (Step 2 ハードン) | ✅ 完了 | (i) subscribe-after-start race fix: aggregator 先登録 → adapter.subscribe → 失敗時 rollback、subscribe idempotent 化。(ii) `_run` 例外時 silent dead fix: `_task.done()` 検査で restart 可能化、`last_error` プロパティで原因取得可能。2 件追加 PASS |
...
- ✅ **2a** `DepthUpdate` pass-through: `_run` に `isinstance(evt, DepthUpdate)` 分岐追加、aggregator 迂回で即時 `bus.publish`
- ✅ **2b** venue 直送 `KlineUpdate` pass-through: aggregator は trades→bar 用、venue 集約済み bar はそのまま流す
- ✅ **2c** 複数 instrument: `subscribe()` を複数回呼べる構造に。既存の `dict[InstrumentId, ...]` 構造で aggregator は instrument 別に独立確定
- ✅ **2d** 複数 interval: 内部 `dict[InstrumentId, list[TickBarAggregator]]` 化。コンストラクタは `interval_ns: int` (Step 1 互換) と `intervals_ns: Iterable[int]` の排他選択、`_normalize_intervals` で正規化
- ✅ **2e** reducer/DataEngine bridge: `engine/live/reducer_bridge.py` 新設。純関数 `live_kline_to_reducer_kline` / `live_kline_to_replay_time_updated` (ts_ns//1e6 = timestamp_ms = open_time_ms、volume は捨てる) と `LiveReducerBridge` (bus 購読 task で `ReplayTimeUpdated→KlineUpdate` の順で `data_engine.apply_replay_event` を呼ぶ)。`DepthUpdate` は reducer の関心外なので無視。
- ✅ **ハードン サイクル 1**: `subscribe()` を「aggregator 先登録 → adapter.subscribe → 失敗時は登録巻き戻し」順に組み直し、idempotent 化（同一 `instrument_id` 再呼び出しで no-op）。**理由**: 旧実装は `await adapter.subscribe(...)` を先に行うため、(i) adapter 側で tick が即流れ込むケースで aggregator 不在によりドロップ、(ii) `start()` 後に `subscribe()` を呼ぶと race で初回 tick を取りこぼす可能性があった。`test_live_runner_subscribe_after_start_does_not_drop_first_tick` がこの race を再現し RED→GREEN で確認。
- ✅ **ハードン サイクル 2**: `start()` の前提を `self._task is not None` から `self._task is not None and not self._task.done()` に修正、`_run` で `CancelledError` 以外の例外を `self._last_error` に保存、`last_error` プロパティで外部から取得可能化。**理由**: 旧実装は `_run` task が例外で死ぬと `_task` が done 状態のまま残り、再 `start()` が no-op になって runner が silent dead になっていた。例外原因も外部から観測不可能。`test_live_runner_can_restart_after_run_task_dies_with_exception` が「`exploding_events()` で死 → `last_error == boom` 確認 → adapter.events を正常版に戻して再 start → 新 tick が bus に流れる」を検証。

設計判断 (Step 2 全体):
- LiveRunner の `subscribe()` で adapter には `{"trades", "depth"}` を購読要求するよう変更（Step 1 は `{"trades"}` のみ）。mock の inject gating は instrument 単位なのでテストは引き続き通る。実 venue では trades / depth で endpoint が違う可能性があり、その場合 adapter 側で内部的に複数 WebSocket を張ることになる（adapter Protocol の責務）。
- bridge は `data_engine` を Protocol 型 (`apply_replay_event(event)`) で duck-typing し、テストでは `_RecordingDataEngine` stub に差し替え。実 `DataEngine` 接続は Phase 8 の後続 (server_grpc 側で `LiveRunner.bus` と `DataEngine` を ExecutionMode に応じて結線) で行う。
- `live_kline_to_reducer_kline` で volume を捨てるのは現 reducer `KlineUpdate(dataclass)` に volume 欄が無いため。Replay 路でも volume を扱わない（`nautilus_adapter.bar_to_kline_update` 参照）ので live/replay の対称性は維持。
- ハードン サイクル 2 の error 観測は **bus への error publish ではなく runner.last_error プロパティ**を採用。理由: `LiveEvent` discriminated union (`KlineUpdate | TradesUpdate | DepthUpdate`) を error イベント追加で広げると、(i) bus の consumer 全員が type narrowing で error case を扱う義務が出る、(ii) replay 系統の `KlineUpdate` 路に error が混ざる可能性が出る。runner side-channel に閉じれば server_grpc から `runner.last_error` を poll するだけで済む。
- ハードン サイクル 2 で `_run` の `except BaseException` は **intentional**: `SystemExit` / `KeyboardInterrupt` も含めて adapter 側の死因を残したい（実 venue 接続中の Ctrl-C などを silent に隠さない）。`CancelledError` だけは graceful stop のシグナルなので別分岐で early return。
...
### 9.5 なぜ `MockVenueAdapter` は Protocol を変えずに mock 固有メソッドで拡張するか（Step C ADR）

`MockVenueAdapter` のテスト制御 API は **Protocol 外** の同期メソッド `inject_tick(event: LiveEvent)` と `emit_depth_snapshot(instrument_id, ts_ns, bids, asks)` で提供する。`LiveVenueAdapter` Protocol 自体（`login` / `logout` / `fetch_instruments` / `subscribe` / `unsubscribe` / `events` / `venue_id`）には一切手を入れない。

選択肢の検討:

- A) Protocol に `inject_tick` を足す → tachibana/kabu 実装でも no-op 実装が必要になり、live と test の境界が Protocol に染み出す
- B) `connect/disconnect/subscribe_klines/subscribe_trades` のような mock 専用 API を作る → live_runner で実 venue と差し替え不可能になり Mock の存在意義（差し替えテスト）が消える
- C) **採用**: Protocol 厳密準拠 + mock 固有メソッドで補う → live_runner からは Protocol のみ叩く、テストからは mock 固有メソッドで制御を握る

Step C 開始時に「現行 Protocol に縛られず別 API で書くか」で実際に Navigator が分岐しかけたので明文化する。**Mock を書くときは、まず `python/engine/live/adapter.py` の現行 Protocol を Read してから RED を書く** (Tips の Navigator 注意項にも記載)。

また `is_logged_in` の状態属性は Protocol に持たせず mock 固有属性とした。理由: Tachibana / kabu の実セッション表現（cookie / token expiry / 仮想 URL set / kabuStation 本体の生死など）は venue ごとに異なり、共通化する意味がない。state machine（VENUE_DISCONNECTED → AUTHENTICATING → CONNECTED）は `live/state_machine.py` 側の責務。

### 9.6 なぜ `inject_tick` は subscribe-gating のみで channel-gating しないか

`MockVenueAdapter.inject_tick(event)` は `if event.instrument_id in self._subscribed:` の gating だけ持ち、subscribe 済み channel と event の `kind` の整合（例: `{"trades"}` だけ subscribe しているのに DepthUpdate を inject）はチェックしない。

理由:

- 「subscribe gating」 = unsubscribe 後の event 漏れを防ぐ防衛線（test_mock_adapter_unsubscribe_stops_flow_to_events で固定）。これは live でも実 venue が WebSocket 切断時に同じ振る舞いを期待される
- 「channel gating」 = test の意思表示の問題。テストが `{"trades"}` を subscribe して `DepthUpdate` を inject したら、それは「テストが意図的にやっている」状況であり、mock が拒否する責務ではない
- 実 venue (tachibana EVENT / kabu PUSH) は channel ごとに別 endpoint / 別 frame で来るので、channel 整合は venue 実装側の責務

`emit_depth_snapshot` も `inject_tick` の薄い wrapper にしてある（gating 共有）。

### 9.7 なぜ `LiveRunner.subscribe()` は aggregator を adapter.subscribe より先に登録するか（Step 2 ハードン ADR）

`LiveRunner.subscribe(instrument_id)` の中で `await self._adapter.subscribe(instrument_id, channels)` を **先に** 行うと、(i) adapter 側で tick が即流れ込むタイプの venue で、`adapter.subscribe()` の return 〜 aggregator dict 登録の間に到着した最初の tick が `self._aggregators.get(...) is None` で silent drop される、(ii) `runner.start()` が既に走っている状態で `runner.subscribe()` を呼ぶケースで同じ race が再現可能。

対策として subscribe を次の順に組み直した:

1. `self._aggregators[instrument_id] = [...]` を先に登録（in-memory なので raise しない）
2. `await self._adapter.subscribe(instrument_id, {"trades", "depth"})`
3. 2 が raise したら `self._aggregators.pop(instrument_id, None)` で登録を巻き戻して再 raise

同時に idempotent 化: `if instrument_id in self._aggregators: return`。同 instrument を再 subscribe しても aggregator が二重生成されないので、`TradesUpdate` が複数 aggregator に流れて bar が二重に確定する事故も防げる。

このルートは race を強制発火する `test_live_runner_subscribe_after_start_does_not_drop_first_tick` で固定済み。新規 adapter 実装（tachibana / kabu）でも同じ前提が成立するよう、adapter 側 `subscribe()` が同期的に return しても tick が直後に流れ込むことは許容する設計。

### 9.8 なぜ `LiveRunner` は error を bus に publish せず `last_error` プロパティで露出するか（Step 2 ハードン ADR）

`_run` task が `adapter.events()` から `CancelledError` 以外の例外で死んだとき、3 つの選択肢があった:

| 案 | 採否 | 理由 |
|---|---|---|
| A: bus に `Error` イベントを publish | ❌ 不採用 | `LiveEvent` discriminated union (`KlineUpdate \| TradesUpdate \| DepthUpdate`) を error case で広げると、replay 系統と共有している型の意味論が崩れ、bus consumer 全員が error 分岐を持つ義務が出る |
| B: runner 自身が例外を再 raise | ❌ 不採用 | background task 内で raise しても誰も catch しない（`asyncio.create_task` の "unhandled exception" warning が出るだけ）。`runner.stop()` の `await self._task` で再 raise させる手もあるが、stop 時点まで原因が露出しない |
| C: side-channel (`self._last_error` + `last_error` property) | ✅ 採用 | bus の型と意味論を一切汚さず、server_grpc が `runner.last_error` を poll するだけで原因取得可能。`start()` を `_task.done()` 受理に変更すれば re-arm も簡潔 |

`_run` 内の `except BaseException` は **意図的**。`SystemExit` / `KeyboardInterrupt` を含めて死因を残したい（実 venue 接続中の Ctrl-C や OOMKill を silent に隠さない）。`CancelledError` は graceful stop シグナルなので別分岐で early return し、`last_error` には保存しない。

`start()` の前提を `_task is not None and not _task.done()` にしたことで、外部監視（例: server_grpc 側の health check）が `last_error` を見て restart を判断できる: `if runner.last_error is not None: runner.adapter = new_adapter; await runner.start()` のような自動回復パターンも可能になる（実装は Phase 8 後続）。

### 9.9 なぜ `LiveRunner._run` は event 種別判定の前に `_is_subscribed` で gating するか（Step 2 レビュー反映 ADR）

レビューで「`DepthUpdate` / `KlineUpdate` の pass-through が runner 側の購読集合で gate されていない」と指摘された。`TradesUpdate` は `_aggregators.get(...) is None` で未購読銘柄を捨てていたが、depth / kline はそのまま `bus.publish` していたため、mock のテストでは見えないが**実 adapter で次の状況**が発生し得た:

1. 実 venue が **global stream** を持っており、特定 instrument を subscribe しなくても全銘柄の depth/kline が流れてくるタイプ（一部 crypto exchange）
2. `unsubscribe` 直後の **残留 frame**（venue 側のキューに残っていた未送信 event が遅れて到着）
3. 別 instrument を subscribe している heredoc 系 adapter で、共有 socket から無関係 instrument の event が混ざる

これらの状況で reducer bridge まで未購読 instrument の data が流れると、`DataEngine` の `ReducerState` に意図しない bar が積まれて **戦略の判断材料が壊れる** 重大事故になる。

対策として `_run` の最初に `_is_subscribed(evt.instrument_id)` で一律 gate し、未購読なら `continue`:
...
`_is_subscribed(id) -> bool` を helper にすることで「未購読は流さない」という契約が `_run` の入り口で 1 箇所で読める。adapter 側 gating （mock の `inject_tick` instrument check）には依存せず、runner レベルで防衛線を持つのがポイント。

### 9.10 なぜ `LiveReducerBridge` にも `last_error` + `_task.done()` 受理を入れたか（Step 2e ハードン ADR）

§9.8 で LiveRunner に入れた「`_task` lifecycle 問題」が **bridge 側にも残っていた**ことがレビューで指摘された。bridge の `_run` は `data_engine.apply_replay_event(...)` を直接呼ぶので、実 `DataEngine` が `replay_state` 検査などで例外を投げた瞬間 task が死に、`_task` が done のまま非 None で残り、再 `start()` が no-op になる silent dead が発生し得た。

選択肢:

| 案 | 採否 | 理由 |
|---|---|---|
| A: bridge は one-shot と明記し再 `start()` を禁止 | △ 検討 | server_grpc が ExecutionMode を切替えるたびに bridge 再生成すれば成立するが、`bus` インスタンスは継続のため bus subscriber list の生死管理が増える |
| B: LiveRunner と同パターン (`_task.done()` 受理 + `last_error`) | ✅ 採用 | runner と semantics を統一でき、覚えることが減る。`_run` の `except BaseException` も §9.8 と同じ理由で intentional |

実装は LiveRunner と完全に同型:
...
- **Driver の Write/Edit が CP932 で書き出すことがある（Step C で発生）**: PowerShell 環境下で `Write` ツール経由の文字列が CP932 (Shift-JIS) で保存され、Python が UTF-8 として decode できず `SyntaxError: 'utf-8' codec can't decode byte 0x8c` で collection error になる事案あり（Step C-5 で発生、docstring の日本語が原因）。対処: (a) Navigator は新規 docstring の日本語が多い場合 ASCII / 英語に倒す、(b) Driver は書き出し前に encoding 設定を確認、(c) 既存ファイルへの Edit は元 encoding を継承するので安全だが、新規ファイルや大きな置換は要注意。事後検証は `python -c "open('path','rb').read().decode('utf-8')"` で即確認可能
- **`events()` の async generator テストは `asyncio.wait_for(it.__anext__(), timeout=...)` で受ける**: `async for evt in adapter.events(): break` だと break までに無限待ちのリスクが残るし、queue が空のときハングする。`asyncio.wait_for(it.__anext__(), timeout=1.0)` で「届かなければ TimeoutError」を明示する形が安全。`unsubscribe` 後の no-flow 確認は逆に `with pytest.raises(asyncio.TimeoutError):` で TimeoutError を expect する pattern（Step C-4b で確立）
- **`fetch_instruments` を deterministic にする際は frozen `InstrumentRaw` を毎回 new でよい**: pydantic v2 の frozen BaseModel は field 等価で `==` が True になるため、`return [InstrumentRaw(...), InstrumentRaw(...)]` を毎回新規生成しても `assert first == second` は通る。module-level constant にする必要なし
- **pair-relay 進行中に Navigator が 1 ターンで RED+GREEN をまとめることがある**: 司令塔は Navigator の指示を要約・改変しないので、そのまま Driver に渡す。Step C-5 は Navigator が「RED+GREEN を 1 ターンに合体」を選択し、結果的に encoding 問題で 1 往復追加が発生したが、原則として Navigator の判断は尊重する
- **Step 1 LiveRunner で確認した async pipeline テストパターン**: `MockVenueAdapter` を `login → runner.subscribe → bus.subscribe → runner.start → adapter.inject_tick × N → asyncio.wait_for(it.__anext__(), timeout=1.0) → runner.stop` の順で組むと deterministic に bar 確定を観測できる。`runner.stop()` を `try/finally` の `finally` 側に置くと、assertion 失敗時も background task が leak しない（Step 1 で確立）。bar 確定は「同分内 tick だけ」では起きず、**次分 tick が 1 本届いた時点で直前 bar が emit される**ことに注意（aggregator の境界跨ぎ判定）。テスト docstring に「close=90」のような次分 tick 値を期待として書きそうになるが、実 assert は同分内最終 price = 110 になる（aggregator §8「close は最後着順」ADR）。
- **Step 2 マルチ interval テストの bucket 揃え方**: `intervals_ns=[1m, 2m]` のように複数 interval を流すとき、KlineUpdate には interval 識別フィールドが無いので bar の出処は `(ts_ns, volume)` の組で同定する。`base_ns = K * lcm(1m, 2m) = K * 2m` 形で全 interval の bucket 境界を一致させ、テスト docstring に「(base,1)=1m@base / (base,2)=2m@base / (base+1m,1)=1m@base+1m」のような matrix を明示する。同 ts_ns 内では 1m のほうが volume=1 で先 (ループ順)、2m が後だが、ロジックは順序非依存にする（set 比較）。
- **Step 2e reducer 接続層 (`live/reducer_bridge.py`)**: live `KlineUpdate(pydantic, ts_ns)` と reducer `KlineUpdate(dataclass, timestamp_ms, open_time_ms)` の型差を **converter 純関数 + bus 購読 task** に分けたのが効いた。bridge テストは `dataclass _RecordingDataEngine` を stub にして `apply_replay_event` 呼び出し順序を直接 assert する（実 `DataEngine` を使うと replay 状態機械の prerequisites が複雑になる）。`bridge.start()` の中で **`bus.subscribe()` を `await create_task` 前に同期実行** する起動順序が必須（§7 ADR 通り）— task spawn 後に subscribe するとレースで初回 publish を取りこぼす。`asyncio.sleep(0.05)` で publish 反映を待つパターンは flaky なので本番では避け、test 内のみ許容。
- **Step 2 ハードン: subscribe race を「強制発火」させるテストパターン**: 通常 race は再現困難だが、`adapter.subscribe` をモンキーパッチで wrap して **「親 subscribe が return した直後・aggregator 登録前のタイミング」で tick を inject + `asyncio.sleep(0)` で _run に制御を強制委譲** すれば deterministic に race が起きる:

  ```python
  original_subscribe = adapter.subscribe
  async def racing_subscribe(instrument_id, channels):
      await original_subscribe(instrument_id, channels)
      adapter.inject_tick(_tick(...))   # aggregator 登録前に投入
      adapter.inject_tick(_tick(...))
      await asyncio.sleep(0)             # _run に制御を渡してドロップを発火
  adapter.subscribe = racing_subscribe
  await runner.start()
  await runner.subscribe("7203.TSE")     # 旧実装ではここで race
  ```

  このパターンは aggregator 順序が間違っていれば確実に TimeoutError で落ちる。新規 race fix を入れたら、このパターンで RED→GREEN を作るのが安全。
- **Step 2 ハードン: `_run` が exception で死んだことを deterministic に再現する方法**: `adapter.events` を `async def exploding_events(): raise boom; yield` に差し替え、`runner.start()` 後に `for _ in range(5): await asyncio.sleep(0)` で event loop を回せば _run が確実に die する。`runner._task.done()` を assert して precondition を固定してから `runner.last_error` を検査するのが flaky 回避のコツ。restart 検証時は `adapter.events = MockVenueAdapter.events.__get__(adapter, MockVenueAdapter)` で bound method を元に戻せる（`__get__` で descriptor protocol を手動適用）。
- **Step 2 ハードン: `except BaseException` を書く判断基準**: ライブラリ / utility では `except Exception` が原則だが、**runner / supervisor 層では `except BaseException`** が正解になることがある。`SystemExit` / `KeyboardInterrupt` を catch しないと、Ctrl-C や OOMKill が background task 内で起きたとき "task が突然消えるだけ" になり原因が分からない。本実装は `except asyncio.CancelledError` → graceful stop 用に別分岐、`except BaseException` → `self._last_error` 保存、の 2 段構え。`server_grpc` 側で同じ pattern を踏襲すること。
- **Step 2 レビュー反映: gating テストで「mock の adapter 直叩き」で未購読 instrument を再現する**: runner.subscribe しない instrument の event を inject したいが、mock の `inject_tick` は `_subscribed` で gating される。回避策は `await adapter.subscribe("9984.TSE", {"depth"})` を adapter level でのみ呼んで mock の gating を通過させ、runner._aggregators には載せない状態を作ること。これで「実 venue が global stream で別銘柄を流してきた」状況と等価な test fixture になる。RED→GREEN 確認後、未購読 event が bus に届かないことを「次に subscribed instrument の event を流して、bus 最初の event が後者であること」で確認する間接 assertion パターン（TimeoutError ではなく `event.instrument_id` を比較できるので flaky 化しない）。
- **Step 2e ハードン: bridge 再 start テストでは `_data_engine` 自体を差し替えてよい**: `_ExplodingDataEngine` で _run を殺した後、`bridge._data_engine = _RecordingDataEngine()` で内部 attr を直接書き換えて restart 確認する。Phase 8 ではテスト目的に限った内部書換を許容（adapter 側でも同じパターンを Step 2 ハードン サイクル 2 で使った）。本番では server_grpc が新しい bridge instance を作る運用なので、外向き API としては `_data_engine` setter は提供しない。
...
### 3.3 Backend: live_runner.py ✅ Step 1 + Step 2 完了 / 多 venue・実 DataEngine 結線は Phase 8 後続
...
> - ✅ **`live/live_runner.py` (Step 1)** — `LiveRunner(adapter, interval_ns)`。`subscribe(instrument_id)` で adapter に `{"trades"}` を購読し instrument 毎に `TickBarAggregator` を 1 個生成。`start()` で `adapter.events()` を消費する background task を spawn、`TradesUpdate` を `aggregator.on_tick` に流し、確定 bar を `self.bus.publish(KlineUpdate)`。`stop()` は task を cancel→await→`bus.close()` の順で graceful。`TradingNode` は使わない（誤発注の構造防止）。1 test passed (`test_live_runner_aggregates_ticks_into_kline_via_bus`)。**bar 確定タイミングは「次分 tick 受信時に直前 bar を emit」**で、close は同分内 last-write-wins（aggregator 仕様 §8 ✅）に従う。
> - ✅ **`live/live_runner.py` (Step 2)** — Step 1 を拡張。`_run` に `DepthUpdate` / `KlineUpdate` (venue 直送) の pass-through 分岐追加、`subscribe()` は `{"trades", "depth"}` を要求するよう変更（mock の gating は instrument 単位なので既存テスト互換）。コンストラクタを `interval_ns: int | None` と `intervals_ns: Iterable[int] | None` の排他選択に変更し、内部 `_aggregators: dict[InstrumentId, list[TickBarAggregator]]` で **instrument × interval マトリクス** を保持。1 tick が同一 instrument の全 interval aggregator に流れる。Step 1 シグネチャ (`interval_ns=INTERVAL_NS`) は backwards-compat テストで保証。+5 tests passed (depth pass-through / direct KlineUpdate pass-through / multi-instrument / multi-interval / single-interval backwards-compat)。
> - ✅ **`live/live_runner.py` (Step 2 ハードン)** — レビュー駆動で 2 サイクル追加。**(i) subscribe race fix**: `subscribe()` を「aggregator 先登録 → `await adapter.subscribe()` → 例外時に登録 rollback」順に変更、同 instrument 再呼び出しで idempotent no-op。`start()` 後の `subscribe()` で初回 tick がドロップしない保証を `test_..._subscribe_after_start_does_not_drop_first_tick` で追加（adapter.subscribe を racing wrapper にラップし aggregator 登録前に tick を queue 投入してドロップ race を強制発火）。**(ii) restart-after-die fix**: `_run` の `except BaseException as exc:` 分岐で `self._last_error = exc` 保存、`start()` を `_task is None or _task.done()` 受理に変更、`last_error` プロパティ追加。`exploding_events()` で _run を殺して `last_error == boom` 確認後、events を正常版に戻して再 start → 新 tick が bus に流れることを `test_..._can_restart_after_run_task_dies_with_exception` で確認。+2 tests passed。
> - ✅ **`live/reducer_bridge.py` (Step 2e)** — `live_kline_to_reducer_kline(live)` / `live_kline_to_replay_time_updated(live)` の純関数で **ts_ns//1e6 = timestamp_ms = open_time_ms** に変換（volume は reducer の dataclass `KlineUpdate` に欄が無いため捨てる、replay の `nautilus_adapter.bar_to_kline_update` と対称）。`LiveReducerBridge(bus, data_engine)` は bus を `start()` で同期 subscribe してから task を spawn し、`isinstance(evt, LiveKlineUpdate)` の都度 `data_engine.apply_replay_event(ReplayTimeUpdated)` → `apply_replay_event(ReducerKlineUpdate)` の順で流す（replay §4.3 順序不変条件と一致）。`DepthUpdate` は無視。`data_engine` は duck typing (`apply_replay_event(event)` Protocol)。5 tests passed。
> - ⏳ Step 2 スコープ外 (Phase 8 後続で対応): server_grpc 側で `ExecutionMode == LiveManual/LiveAuto` のときに `LiveRunner` + `LiveReducerBridge` を `DataEngine` と結線する wiring、複数 venue 並走、実 venue I/O 本体 (tachibana/kabu HTTP/WS)。
...
### 3.5 Rust UI: 接続フロー & 表示 ✅ 完了 (menu_bar + §3.5.1 SetExecutionMode RPC + sidebar Tickers/SelectedSymbol/SubscribeMarketData)
...
#### 3.5 完了報告 (2026-05-18, HEAD `3b7e0d9`)

Phase 8 §3.5 の menu_bar 配線 + §3.5.1 `SetExecutionMode` RPC + §3.6 File→New / Open Strategy mode 依存配線が完了。

- **配線完了範囲**:
  - `src/ui/menu_bar.rs`: `MenuTopLevel::Venue` + 5 サブ項目 (Connect Tachibana Demo/Prod / Connect kabuStation Verify/Prod / Disconnect)、Alt+V でメニュー展開、`VenueConnectRequested(venue_id, env_hint)` / `VenueDisconnectRequested` イベント → `VenueLogin` / `VenueLogout` RPC 発火
  - `File → New` [新設]: `SetExecutionMode(LiveManual)` を先行発火 + Replay 稼働中なら `ForceStopReplay` を先行発火してから戦略 buffer 破棄
  - `File → Open Strategy`: ExecutionMode 依存分岐。Live (Manual/Auto) なら `SetExecutionMode(LiveAuto)` を先行発火してから `.py` ロード、Replay なら Phase 7 と同じ `LoadStrategy` + Replay State Machine 経路
  - §3.5.1 Footer `ExecutionModeToggle`: クリックを `SetExecutionMode` RPC 経由に切替 (旧: ローカル `ExecutionModeRes` 直書き)。precondition reject (LIVE_ADAPTER_NOT_CONFIGURED 等) は backend からの error_code をログ出力
  - `BackendTradingState` に `venue_id: Option<String>` / `instruments_loaded: Option<u32>` (両者 `#[serde(default)]`) 追加。`main.rs` polling task で `VenueChanged { state, venue_id, instruments_loaded }` の placeholder を `state.venue_id.clone()` / `state.instruments_loaded.unwrap_or(0)` に置換
  - Python `TradingState.instruments_loaded: int` field 追加。`get_current_state` / `take_snapshot` で `len(_subscribed_instruments)` を充填 (live runner 配線時に自然な値に育つ)
- **検証**:
  - `cargo check` / `cargo test --lib` **208 passed** (新規 warning ゼロ) / `cargo build --release` green
  - `cd python && uv run pytest -m "not slow"` **719 passed** (回帰なし)
- **Phase 8 follow-up に繰越**:
  - §3.5 sidebar: `Tickers` Resource / `SelectedSymbol` Resource / `ListInstruments(source="local")` 呼び出し / 銘柄クリック時の `SubscribeMarketData` 発火 (これらの型/RPC が未定義のため別 PR で proto 追加から)
  - `File → New` 時のサイドカー `.json` auto-save (Phase 7 サイドカー機構の流用設計が未確定)
  - `File → New` / `Unload` 時の戦略 buffer 安全 unload (cosmic-text buffer の lifecycle 整理が別 issue)
  - `Venue → Disconnect` 時の ExecutionMode=Live 確認ダイアログ (案 C 採用により `ModalLayer` 配線を本セッションでは見送り、Disconnect は即時実行)
- **ユーザー確定事項の踏襲**:
  - `LiveAuto` 切替は Phase 8 では UI ハイライト + RPC 発火のみで、実際の発注経路結線は Phase 10
  - menu_bar の Venue 確認ダイアログは ModalLayer 整備後 (Phase 8 follow-up) に追加
  - 旧 `ExecutionModeToggle` の楽観的ローカル更新は backend single source of truth に揃えるため削除済み (commit `7a831ac` `fix(ui): route ExecutionMode toggle through SetExecutionMode RPC (no optimistic state)`)

#### 3.5 sidebar 完了報告 (2026-05-18)

Phase 8 §3.5 の sidebar Live universe 配線 (Tickers 上書き / 検索 / 仮想スクロール / mode 依存クリック) が完了。

- **proto / backend 拡張**:
  - `python/proto/engine.proto` に `message Instrument { id; name; market }` 追加。`ListInstrumentsResponse` に `repeated Instrument instruments = 4` を追加 (既存 `instrument_ids` は backwards-compat のため残置)
  - `python/engine/server_grpc.py` `ListInstruments` で Replay catalog dir 名から id 抽出後、`Instrument{id, name=id, market=""}` で `instruments` field を埋める (Live venue adapter による name/market 充填は Phase 8 follow-up に繰越)
  - Python stub 再生成。`from . import engine_pb2` への手当て (proto 再生成のたび absolute-import に戻る既知罠)
- **Rust core 配線範囲**:
  - `src/trading.rs`: `Ticker { id, name, market }` struct + `Tickers` Resource (`Vec<Ticker>`) + `SelectedSymbol` Resource (`Option<String>`) 追加
  - `src/trading.rs`: `TransportCommand::ListInstruments { source: Option<String> }` / `TransportCommand::SubscribeMarketData { instrument_id }` 追加
  - `src/trading.rs`: `BackendStatusUpdate::InstrumentsListed { instruments }` 追加 (上書きセマンティクス、§0.5.1)
  - `src/main.rs`: `apply_status_update` に `InstrumentsListed` arm + `&mut Tickers` 引数追加 (test 側 `apply` helper も一括追従)
  - `src/main.rs`: `fire_list_instruments` ヘルパー追加。初期接続後に 1 回 + `VenueState::Connected`/`Subscribed` 遷移時に再 fire (Live universe 上書き、§3.5 1001)
  - `src/main.rs`: `TransportCommand::SubscribeMarketData` ハンドラ追加 (channels=`["trades","depth"]`、precondition reject は warn のみ — 案 C / ModalLayer 未存在)
- **sidebar UI 配線範囲** (`src/ui/sidebar.rs`):
  - `Tickers` section [新設] = 検索ボックス (click→focus / `tickers_search_input_system` が `Events<KeyboardInput>::drain` で picker_searchbox と同パターン消費 / Esc で clear+unfocus) + 仮想スクロール viewport (`TICKERS_VISIBLE_ROWS=18` 行固定 spawn、`SidebarTickersScrollOffset` を mouse wheel で更新、`filter_tickers` + `clamp_scroll_offset` の純関数ヘルパーは unit test 済み)
  - `ticker_row_click_system`: `ExecutionMode == Replay` → `SelectedSymbol` 更新のみ / `Live*` → `SelectedSymbol` 更新 + `TransportCommand::SubscribeMarketData` enqueue (§3.5 1009-1010 通り)
  - 行 BG は selected 時にハイライト (`TICKER_ROW_SELECTED`)、行ラベルは `id` (`name` が `id` と異なるときのみ `"id  name"` 形式)
  - 既存「Instruments」セクション (Phase 7.5a の scenario 編集リスト) は影響範囲外として並存
- **検証**:
...
### 3.6 UI: モード関連の UX フロー ✅ 完了 (File→New / Open Strategy mode 依存配線完了、サイドカー auto-save / Disconnect 警告 / 戦略 buffer unload は Phase 8 follow-up)
...
> **Phase 8 スコープ外。** Phase 8 は read-only な市場接続に留め、発注 UI は `PlaceOrder` RPC（Phase 9）と同時に実装する。
...
Phase 8 マージ後のコードレビューで指摘された 4 件を修正。`cargo test --workspace`
全 GREEN (305 + 21 + 2 = 328 tests)。

#### UI

- **HIGH-1: `instruments_universe_prune.rs` の Live branch に empty-list guard を追加**
  Live 分岐は `tickers.list.is_empty()` でも空の `allowed` HashSet を作り
  `InstrumentRegistry` を全削除していた。Live universe フェッチが一時的に `[]` を
  返したときに全銘柄が消える事故を防ぐため、Replay branch と同じく早期 `return`
  を入れた（`debug!` で skip 理由を残す）。新規テスト
  `prune_skips_when_live_tickers_list_is_empty`.

- **HIGH-2: `is_venue_busy_for_menu` に `Reconnecting` と `Error` を含める**
  これらの状態は backend 側でも slot を占有しており、新規 `VenueLogin` を発火させると
  adapter で衝突する。`Error` は Disconnect ボタンが残るため UX 上は遷移可能。
  新規テスト 3 件（`*_reconnecting`, `*_error` を busy 側に修正、
  `*_all_variants` で全列挙固定）。

- **MEDIUM-1: footer.rs Replay state badge / gRPC status の blind write を差分書き込みに変更**
  Bevy 規約 2（change-detection thrash 回避）違反。`state_q` / `grpc_q` の Text と
  TextColor を毎フレーム比較してから書き込むようにし、format 文字列も loop 外で 1 回
  だけ組み立てるよう整理。sidebar.rs:399 は既に diff-check 済みで対応不要。

#### Backend pump

- **MEDIUM-2: `SetExecutionMode` を `tokio::spawn` 化**
  `set_execution_mode(req).await` を pump ループ上で直接 await していたため、
  mode 切替中は後続コマンド（`FetchAvailableInstruments` 等）が詰まっていた。
  sibling コマンドと同じ `tokio::spawn` パターンに合わせた。
  `tests/backend_integration.rs` の 2 ケースは GREEN 維持。

#### Tachibana adapter (2026-05-20 追加)

`cd python && uv run pytest tests/exchanges/test_tachibana*.py` 195 passed / 1
skipped。`uv run pytest -m "not slow"` は 851 passed（残 4 件は kabu Phase 9 系の
pre-existing failure で本修正範囲外）。

- **HIGH-1: `fetch_instruments` で server-side error envelope を握り潰さない**
  `p_errno="-62"` や `sResultCode="-1"`、session-expired (`p_errno="2"`) のエラー応答が
  ストリーム完了前に単発で返る場合、これまで `[]` を返して隠蔽していた。`parser.is_complete=False`
  かつ最初の record に `p_errno` / `sResultCode` を含むケースで `tachibana_auth.check_response`
  を呼んで `SessionExpiredError` / `ApiError` を再生（R6 二段判定）。RED: `test_fetch_instruments_raises_on_session_expired` /
  `test_fetch_instruments_raises_on_api_error` / `test_fetch_instruments_raises_on_service_out_of_hours`.

- **HIGH-2: master fetch の incremental decoder を `errors="strict"` 化（R7）**
  `tachibana_master.MasterStreamParser` の SJIS `IncrementalDecoder` を `errors="replace"`
  から `errors="strict"` に変更。`tachibana_codec.decode_response_body` 経由のログ用途は
  引き続き `replace` を許容する API のまま。RED: `test_master_parser_strict_decode_rejects_invalid_sjis`,
  `test_master_parser_strict_decode_accepts_valid_sjis`.

- **MEDIUM-1: `_validate_virtual_urls` の error log を `mask_secrets()` 経由に（R3/R10）**
  仮想 URL は `ND=<session token>/` の secret を含むため、`%r url` の生出力は禁止。
  `engine.live.logging.mask_secrets` を経由して `{key: "***"}` 形式でログするよう変更
  （regex は既に `sUrl[A-Z]` をカバー済み）。RED: `test_validate_virtual_urls_does_not_leak_url_in_log`.

- **MEDIUM-2: `events()` が `logout()` 後に永久ハングしない**
  `logout()` で `self._queue.put_nowait(None)` の sentinel を投入し、`events()` 側で
  `None` 受信時に `return`（StopAsyncIteration）するようにした。`mock_adapter` と同じ
  パターン。RED: `test_events_returns_after_logout` (`asyncio.wait_for(..., timeout=1.0)`).

- **MEDIUM-3: master fetch の read timeout を 60s→600s（multi-MB stream 対応）**
  `_MASTER_READ_TIMEOUT = 600.0` をモジュール定数として導入。kabu master は数 MB あり、
  低速回線で 60s では完走しないケースがあったため。RED: `test_master_read_timeout_is_at_least_300s`.

- **MEDIUM-4: `credentials_source="prompt"` を `tachibana_login_flow.run_dialog` に配線**
  `NotImplementedError` を撤去。`run_dialog(env_hint=self._env)` を呼び、`success=True` なら
  `load_session()` で TachibanaSession を構築（session_cache 経路と同じロジック）、
  `success=False` なら `ValueError(error_code)` を投げる。RED: `test_login_prompt_success_loads_session`,
  `test_login_prompt_user_cancel_raises`. 既存の `test_login_prompt_raises_not_implemented`
  は仕様変更により削除。

#### Round 2 — Tachibana (再レビュー)

`cd python && uv run pytest tests/exchanges/test_tachibana*.py` 198 passed / 1 skipped。
`uv run pytest -m "not slow"` は 879 passed（残 4 failed + 1 error は kabu adapter の
pre-existing failure で Round 2 スコープ外、stash 検証済み）。

- **HIGH-1: `logout()` の `None` sentinel が次セッションへ漏洩する問題を修正**
  `logout()` は `_queue.put_nowait(None)` で events() consumer を起こすが、`login()`
  経路（env / prompt / session_cache）が `_queue` を再生成しないため、
  `logout() → login() → events()` を回すと新しい consumer が前セッションの stale
  `None` を pop してすぐ終端していた。`login()` 冒頭で `self._queue = asyncio.Queue()`
  を再生成（drain ではなく recreate — 旧 queue を握っている producer task と物理的に
  切り離す）。RED: `test_events_after_relogin_does_not_see_stale_sentinel`
  (`asyncio.wait_for(timeout=1.0)`).

- **MEDIUM-1: master fetch の `UnicodeDecodeError` を `ApiError("MASTER_DECODE_FAILED")` に変換**
  `MasterStreamParser.feed(chunk)` は `errors="strict"` の SJIS IncrementalDecoder を
  使うため、不正バイトで `UnicodeDecodeError` を投げる。これが gRPC ハンドラまで生で
  伝播していたので、`fetch_instruments` の `parser.feed(chunk)` を try/except でラップし
  `ApiError("MASTER_DECODE_FAILED", str(exc))` を `from exc` で再 raise。原因は
  `__cause__` で保持。RED: `test_fetch_instruments_wraps_unicode_decode_error_as_api_error`.

- **MEDIUM-2: error envelope 走査を `records[0]` から full scan に変更**
  従来は最初の record だけ `p_errno` / `sResultCode` をチェックしていたが、サーバが
  CLMEventDownloadComplete 終端なしで「valid record → error envelope」の順で返した場合に
  envelope を取りこぼす。`for rec in records` で full scan し、最初に `p_errno` /
  `sResultCode` キーを持つ record を `tachibana_auth.check_response(...)` に渡す。
  RED: `test_fetch_instruments_scans_all_records_for_error_envelope`.


### Post-merge review fixes — backend orchestration (2026-05-20)

Phase 8 backend orchestration のコードレビューで指摘された 7 件を修正。
`cd python && uv run pytest -m "not slow" -q` で 824 passed / 3 skipped / 67 deselected GREEN
（事前から working tree で fail していた `tests/exchanges/test_kabusapi_adapter.py` の 4 件は別レビュースコープ・本修正の対象外）。

- **HIGH-1: `TradingState.live_last_error` を末尾フィールドに移動（§9.14 ADR）**
  `models.py` で `last_prices` → `live_last_error` の順に並び替え。Rust デシリアライザが
  末尾追加 optional として扱う前提を満たす。RED: `test_trading_state_live_last_error_is_last_field`.

- **HIGH-2: `SubscribeMarketData` に 50 銘柄 cap を導入（§0.3 / kabu API 上限）**
  `server_grpc.SubscribeMarketData` で `runner.subscribed_ids()` を見て 50 件に達したら
  新規追加だけ `SUBSCRIPTION_LIMIT_EXCEEDED` を返す。re-subscribe は idempotent で
  カウント対象外。RED: `test_subscribe_market_data_cap_at_50_instruments`,
  `test_subscribe_market_data_re_subscribe_does_not_count_against_cap`.

- **HIGH-3: `live_last_error` を mode toggle / VenueLogin 成功で suppress（§9.14 ADR）**
  servicer に `_suppress_live_last_error: bool` flag を追加。`_teardown_live_components`
  完了時と VenueLogin 成功時に `True` 化、次の `GetState` で `err is None` ならそのまま
  None を返し、新規 error が出たら自動で `False` に戻す。RED:
  `test_live_last_error_cleared_when_toggling_to_replay`,
  `test_live_last_error_cleared_on_venue_re_login`.

- **MEDIUM-1: `VenueStateMachine` に ERROR → DISCONNECTED 遷移を追加**
  `_ALLOWED["ERROR"] = {"DISCONNECTED"}` に変更。`_fail()` リカバリで `reset()` 経由せず
  明示的に DISCONNECTED へ落とせるようにする。`reset()` 経路は引き続き有効。RED:
  `test_error_to_disconnected_via_transition_to_is_allowed`（既存の
  `test_error_only_resets_to_disconnected_via_reset` は新ポリシーに合わせ rename + assertion 緩和）.

- **MEDIUM-2: `LiveRunner.is_logged_in()` の deny-by-default 化**
  `getattr(adapter, "is_logged_in", True)` → `False` に変更。adapter が attribute を持たない
  場合は「未ログイン」扱い。RED: `test_is_logged_in_defaults_to_false_when_adapter_lacks_attribute`,
  `test_is_logged_in_true_when_adapter_exposes_true`.

- **MEDIUM-3: `mask_secrets()` のキーワード coverage 拡張**
  `cookie` / `set-cookie` / `authorization` / `bearer` / `apiKey`（case 全変種）/
  `SECOND_PASSWORD`（snake/kebab/SCREAM）を `re.IGNORECASE` で吸収。`sUrl[A-Z]` は
  case-sensitive のまま分離した regex で残存（`sUrllower` 非秘密の互換維持）。RED:
  `test_masks_cookie_and_set_cookie_headers`, `test_masks_authorization_and_bearer`,
  `test_masks_apikey_case_variants`, `test_masks_second_password_variants`.

- **MEDIUM-4: `LiveRunner.stop()` で bus を close しない（§9.8 ADR re-arm pattern）**
  `stop()` は task cancel のみ、bus 維持。明示的に破棄したいときだけ `aclose()` を呼ぶ。
  `server_grpc._teardown_live_components_async` は `runner.aclose()` に切替。
  RED: `test_live_runner_restart_after_stop_still_publishes_events` (start→stop→start で
  同じ bus 経由で event が流れる).

- **MEDIUM-5: phase8-live-loop thread に exception handler を追加**
  `_ensure_live_loop` で `loop.set_exception_handler(_loop_exception_handler)` を設定し、
  uncaught asyncio exception を `mask_secrets` 経由で ERROR ログ出力。`loop.run_forever()`
  自体も try/except で囲み、thread silent death を防ぐ。RED:
  `test_phase8_live_loop_logs_uncaught_asyncio_exception` (bare Task で raise → handler が
  ERROR log を出すことを検証).


## Post-merge review fixes (2026-05-20)

`sasa/Phase8-Live-Venue-and-Market-Data` ブランチで kabu adapter の post-merge コードレビュー
指摘を TDD (RED→GREEN) で fix。`cd python && uv run pytest -m "not slow" -q` で
873 passed / 3 skipped GREEN。

- **HIGH-1: `_put_register` を `check_response` 経由に変更（R7 復元）**
  HTTP 4xx と body Code を二段判定に通し、`4002001 → KabuRegisterFullError` /
  `4001005 → KabuTokenExpiredError` を caller に正しく伝播。成功時は `True` のまま。
  RED: `test_put_register_returns_false_on_nonzero_result_code`（assert 書換）、
  `test_put_register_http_401_raises_api_error`、
  `test_put_register_code_4001005_raises_token_expired`.

- **HIGH-2: `_processors` を `(Symbol, Exchange)` tuple キーに変更（R4）**
  Symbol 文字列単独キーだと TSE(1) と 名証(3) の同コードが `_prev_volume` / quote state を
  共有して破壊するため、kabu の `<Symbol>@<Exchange>` 規約に合わせて tuple キー化。
  RED: `test_on_frame_keys_processor_by_symbol_and_exchange`.

- **HIGH-3: WS reconnect 時に processor.reset() を発火する `on_reconnect` hook を追加**
  `kabusapi_ws.connect()` に `on_reconnect` keyword 引数を新設し、2 回目以降の successful
  connect で `put_register` replay の前に caller callback を発火。adapter は
  `_reset_all_processors` を渡し、全 processor の DV/quote state を初期化。
  RED: `test_ws_reconnect_resets_processors`、
  `test_connect_invokes_on_reconnect_before_replaying_register`.

- **MEDIUM-1: `_put_register` に R5 token-bucket（info 10 req/s）を導入**
  `_TokenBucket` を adapter 内に追加し、`_put_register` の先頭で `acquire()`。
  テスト容易化のため `time_source` コンストラクタ引数と `_rate_limit_sleep` 注入点を提供。
  RED: `test_put_register_rate_limited_to_10_per_second`.

- **MEDIUM-2: `logout()` で `PUT /unregister/all` を発火（R6 クリーンアップ）**
  `_register_set` が空でなければ logout 冒頭で `PUT /unregister/all` を best-effort で
  実行。token 失効・本体停止に伴う任意の例外は warning log のみで握り潰し、
  以降の cleanup を継続。RED: `test_logout_issues_unregister_all`、
  `test_logout_tolerates_unregister_all_failure`.

- **MEDIUM-3: WS task 例外を `adapter.last_error` 側チャネルで公開**
  `_run_ws` wrapper で `kabusapi_ws.connect` を `try/except` し、`asyncio.CancelledError`
  以外を `self._last_error` に保存して raise（live_runner §9.8 ADR と同じパターン）。
  `events()` 経由の伝播はそのまま、健康診断用に property 経由で参照可能。
  RED: `test_last_error_exposes_ws_task_exception`.

- **MEDIUM-4: WS 再接続 except 節を network エラーに narrow**
  従来の `except Exception` を `OSError | asyncio.TimeoutError | ConnectionClosed*` に絞り、
  JSON decode error / on_message callback error はフレーム単位の内側 try で warning log
  のみ出して continue（`consecutive_failures` を増やさない）。
  RED: `test_connect_does_not_increment_failures_on_json_decode_error`.

設計判断:
- `on_reconnect` hook は adapter 側に置き、`processor.reset()` の呼び出し責務は adapter
  が持つ（codec docstring が caller の責務と明記）。WS 層はトリガを発火するだけで
  processor 辞書の所有権を持たない。
- `_put_register` 失敗時の rollback は subscribe 側で `try/except BaseException` で
  実施（既存の `was_registered` 巻き戻しロジックを維持）。

### Round 2 (2026-05-20 follow-up)

再レビューで検出した 4 件 (+ 既存テストの teardown ERROR 1 件) を TDD で fix。
`cd python && uv run pytest -m "not slow" -q` で 883 passed / 3 skipped GREEN。

- **HIGH-1: `logout()` の `except BaseException` を `CancelledError` 通過に修正**
  `python/engine/exchanges/kabusapi.py:169`。`PUT /unregister/all` を囲む try/except が
  `BaseException` で `asyncio.CancelledError` まで握り潰し、shutdown semantics を
  壊していた。`except asyncio.CancelledError: raise` +
  `except (httpx.HTTPError, asyncio.TimeoutError, RuntimeError)` の二段に絞り、
  cancellation を caller に伝播するよう修正。
  RED: `test_logout_propagates_cancelled_error_during_unregister_all`.

- **HIGH-2: `PUT /unregister/all` に明示 `timeout=httpx.Timeout(5.0)` を付与**
  body が hang したまま戻らないと logout が無限ブロックする問題。`httpx.HTTPError`
  (= `httpx.TimeoutException` を包含) で握り潰し継続。
  RED: `test_logout_unregister_all_has_timeout`
  (`asyncio.wait_for(timeout=2.0)` で 2s 以内に完了することを確認).

- **MEDIUM-1: `login()` 冒頭で `self._last_error = None` を実行**
  前回 session の WS task 例外が `last_error` に残り、健康診断 caller が古い状態を
  観測する問題。「成功 login は prior session の last_error をクリアする」を契約化。
  RED: `test_login_clears_prior_last_error`.

- **MEDIUM-2: `_on_frame` の Exchange 欠落時 TSE デフォルトを廃止**
  `python/engine/exchanges/kabusapi.py:265`。Exchange 欠落 frame を無条件に TSE(1) に
  流すと silent mis-routing を生む。`_processors` keys から symbol 一致を look up し、
  単一一致は採用、複数一致は warning log + drop、不一致は drop に変更。
  RED: `test_on_frame_missing_exchange_resolves_via_processors` /
  `test_on_frame_missing_exchange_drops_when_ambiguous`.

- **MEDIUM-3: HIGH-3 reconnect test の `_fake_connect` を実 gating に整合**
  `python/tests/exchanges/test_kabusapi_adapter.py:914-922`。`kabusapi_ws.connect` は
  `is_first_connect` gate で 2 回目以降のみ `on_reconnect` を発火するが、テスト fake が
  初回から fire していたため初回 fire の regression を検出できなかった。
  加えて `kabusapi_ws.connect` 直接呼びの「初回は fire しない」テストを新設。
  RED: `test_ws_reconnect_resets_processors_gated_by_call_count`,
  `test_ws_on_reconnect_not_invoked_on_first_connect`.

- ついで: `test_last_error_exposes_ws_task_exception` が `unregister/all` matcher を
  未消費のまま終了し pytest-httpx teardown で ERROR 化していたのを、末尾に
  `adapter.logout()` を追加して matcher を消費するよう修正
  (MEMORY.md `pytest-httpx-unused-matcher-teardown-error` の系列)。

#### Round 3 — kabu (2026-05-20 follow-up)

Round 2 後の再レビューで検出した 3 件の MEDIUM を TDD で fix。
`cd python && uv run pytest -m "not slow" -q` で 888 passed / 3 skipped GREEN。

- **MEDIUM-1: `logout()` の except tuple に `OSError` を追加**
  `python/engine/exchanges/kabusapi.py:183` 付近。`PUT /unregister/all` を囲む
  except が `OSError` / `ConnectionResetError` を拾わず、shutdown race で
  closed/half-open transport が raw OSError を投げると best-effort cleanup を
  突き抜けて caller に伝播していた。`(httpx.HTTPError, asyncio.TimeoutError,
  RuntimeError, OSError)` に拡張。`except asyncio.CancelledError: raise` は
  前段で維持。
  RED: `test_logout_tolerates_oserror_during_unregister_all`.

- **MEDIUM-2: 曖昧 Exchange warning を per-symbol で 1 回に rate-limit**
  `python/engine/exchanges/kabusapi.py:284` (`_on_frame`)。Exchange 欠落 +
  symbol 複数一致 frame が kabu PUSH レートで連続到来すると
  `logger.warning` が毎フレーム発火しログ氾濫を起こす。`self._exchange_ambiguity_warned: set[str]` を
  追加し、初回 occurrence のみ WARNING、以降は DEBUG。`login()` 成功時と
  `logout()` で `.clear()` し、新セッションでは warning を再発火する。
  RED: `test_on_frame_ambiguous_exchange_warns_once_per_symbol`,
  `test_on_frame_ambiguous_warning_resets_on_login`.

- **MEDIUM-3: `login()` の `_last_error = None` を success path 限定に移動**
  `python/engine/exchanges/kabusapi.py:127-131`。冒頭で clear すると
  `prompt_result` の token 欠落 raise や env password 欠落 raise の前に
  状態が消え、別 task の health check が false-healthy を観測する。clear を
  `prompt_result` 成功直前 (`self._token = creds.token` 直前) と env 成功
  直前 (`fetch_token` 後の `self._token = token` 直前) の 2 箇所に分割。
  Credential validation で raise した場合は prior `_last_error` を温存。
  RED: `test_login_does_not_clear_last_error_on_prompt_result_empty_token`
  (+ regression guard: `test_login_clears_last_error_on_success_only`).
