...
`SidecarLayout` のすべての layout フィールドを **`Option<>` 化**し、「キー不在 = no-op」と「キーあり = 適用」を deserialize レベルで区別する。

重要: [`apply_layout_system` (L296 周辺)](../../src/ui/layout_persistence.rs#L296) は **`layout.windows` に含まれないパネルを despawn する** 既存挙動を持つ。scenario-only JSON で `windows` が空配列にデフォルト化されると、Open した瞬間に全パネルが消える事故になる。**`windows: Option<Vec<...>>` として `None` を no-op、`Some(vec![])` を「明示的に全閉じ」として区別する**こと。
...
struct SidecarLayout {
    ...
    /// None = layout キー不在 → apply 全 skip
    /// Some(vec![]) = 明示的に全 window を閉じる
    /// Some(non-empty) = 通常の layout 適用
    #[serde(default)]
...
| 状態           | `viewport`     | `windows`         | apply_layout の振る舞い                                  |
| -------------- | -------------- | ----------------- | -------------------------------------------------------- |
| scenario-only  | None           | None              | カメラ・パネル両方 no-op（既存パネルを despawn しない） |
| layout-only    | Some           | Some(vec![...])   | 正常 apply（既存挙動）                                  |
| 両方入り       | Some           | Some(vec![...])   | 正常 apply                                              |
| 明示的全閉じ   | Some           | Some(vec![])      | 全パネル despawn（既存挙動の意図的維持）                |

`apply_layout_system` 内のロジック分岐:
- `viewport: None` → camera 触らない
- `windows: None` → despawn / spawn どちらもしない
- `windows: Some(list)` → 既存ロジック通り（list に無いパネルを despawn）
- `schema_version: None` → ERROR を出さず debug ログに留める
...
3. **`build_layout` シグネチャを「保存先 path を明示的に受け取る」形に変更**し、scenario を回収するかしないかを呼び出し側が制御する
   - 旧: `build_layout(panels, camera, buffer) -> SidecarLayout`
   - 新: `build_layout(panels, camera, buffer, save_target: &Path, preserve_scenario_from: Option<&Path>) -> SidecarLayout`
   - `preserve_scenario_from = Some(orig)` のとき、その path の既存 sidecar から `scenario` キーを回収して新 layout に含める
   - `preserve_scenario_from = None` のとき、scenario は `None`（運ばない）
4. 影響する save 経路ごとの引数:

| 経路                                  | save 先                                 | `preserve_scenario_from`                    |
| ------------------------------------- | --------------------------------------- | ------------------------------------------- |
| `handle_save_layout_system`           | `<original>.json`（自動）or dialog 選択 | `Some(buffer.original_path)`（同一ファイルなので merge する） |
| `handle_close_autosave_system`        | `<original>.json`                       | `Some(buffer.original_path)`                |
| debounced autosave 系統               | `<original>.json`                       | `Some(buffer.original_path)`                |
| `handle_save_as_layout_system`        | dialog で選んだ別 path                  | **`None`**（明示的に別ファイル化するので scenario を運ばない） |

これにより、レビュー指摘の「現状の `handle_save_as_layout_system` がそのまま `build_layout` を呼ぶと scenario が運ばれる」矛盾を引数で防ぐ。各 save system 実装時に「自分の経路の意図」をシグネチャレベルで宣言させる。

**テスト戦略の注意（auto-load one-shot との関係）**:

`watch_open_strategy_for_sidecar_system` は [`SidecarAutoLoadState.done`](../../src/ui/layout_persistence.rs#L389) により sidecar auto-load を**初回 Open だけ**に制限している（layout restore のループ防止）。2 個目以降の Open では layout loader 自体が走らない。

そのため「7 個の scenario-only JSON を順に Open しても ERROR が出ない」を E2E で検証しようとすると、2 個目以降はそもそも layout loader を通らず AC が空振りする。テスト戦略を以下に分ける:

- **unit / integration テスト**: `LayoutLoadRequested { path }` を**直接発行**して各 JSON 状態を検証する（auto-load one-shot を経由しない）
- **E2E（T5）**: 「初回 Open で scenario-only JSON でも ERROR が出ない」だけを検証する。one-shot 後の挙動は unit に任せる
- **代替案**（実装時に検討）: `SidecarAutoLoadState.done` を「layout restore のループ防止」だけに限定し、scenario-only JSON は別経路で扱う設計に分離する（scope 拡大なので採用判断は T1D 着手時）

**TDD**:

1. RED: `test_deserialize_scenario_only_sidecar` — `{"scenario": {...}}` だけの JSON が `from_str::<SidecarLayout>` で成功し、layout 部分は all None / `windows = None`
...
4. RED: `test_apply_layout_scenario_only_is_noop` — scenario-only（`windows: None`）を apply してもカメラも windows も変化せず、**既存パネルが despawn されない**
4a. RED: `test_apply_layout_explicit_empty_windows_despawns_all` — `windows: Some(vec![])` は既存パネルを全 despawn する（明示挙動の維持）
...
**契約**: Open 後の cache ディレクトリは「元 .py と元 .json の現在のスナップショット」と一致すること。**stale な cache sidecar が残ってはならない**。

**作業**:

cache `.py` を書き込む処理の直後に:

1. **先に `<hash>__foo.json` を unconditional に削除**（stale cache cleanup）
2. 元 `<strategy>.json` が存在するならコピー、存在しなければ削除のみで終了

順序がポイント:「元 sidecar 不在ならスキップ」だけだと、以前 sidecar 付きで Open した後に元 sidecar を削除した場合に `<hash>__foo.json` が残ったままになり、backend が stale scenario を読む事故が起きる（指摘どおり）。
...
if cache_sidecar.exists() {
    if let Err(err) = std::fs::remove_file(&cache_sidecar) {
        warn!("failed to remove stale cache sidecar {:?}: {}", cache_sidecar, err);
    }
}
...
1. RED（integration）: `test_open_strategy_copies_sidecar` — tmp に `foo.py` + `foo.json` を作り、`OpenStrategyRequested` 発行後 `<cache>/<hash>__foo.json` が存在し内容が一致
...
4. RED: `test_reopen_after_deleting_original_sidecar_removes_cache_sidecar` — 1 回目: 元 .py + .json で Open → cache に両方できる。2 回目: 元 .json を削除して同じ .py を再 Open → **cache .json も消える**こと（stale 残留防止）
5. GREEN: 実装
...
- 元 sidecar を消してから再 Open すると cache sidecar も消える
...
| [CRITICAL] scenario-only JSON Open で `windows: []` がデフォルト → 既存パネル全 despawn          | **High**   | **High** | T1D で `windows: Option<Vec<_>>` 化。`None` = no-op / `Some(vec![])` = 明示全閉じ で区別 |
| [CRITICAL] 元 sidecar 削除後の再 Open で stale cache sidecar が残り backend が古い scenario を読む | High       | High    | T1E で cache .py 書き込み時に cache sidecar を**先に unconditional 削除** → 元 sidecar があればコピー |
| Save As が `build_layout` 経由で scenario をコピーしてしまう矛盾                                | Mid        | Mid     | T1D で `build_layout(.., preserve_scenario_from: Option<&Path>)` シグネチャ。Save As は `None` を渡す |
| sidecar auto-load の one-shot 制御で 2 個目以降の Open が AC を空振りする                       | Mid        | Mid     | T3 で `LayoutLoadRequested` 直接発行の unit / integration テストに寄せる。E2E は初回 Open のみで検証 |
