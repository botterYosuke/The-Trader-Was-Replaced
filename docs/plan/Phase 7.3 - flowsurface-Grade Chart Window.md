...
### フェーズ進捗
- ✅ **Phase A0 完了・コミット済** (per-instrument data refactor + volume/depth backend)。レビュー指摘 3 件も A0 内で修正済。検証: `cargo test --lib` 332 passed / `rg '\bTradingData\b|ResMut<\s*TradingData\s*>|Res<\s*TradingData\s*>' src/` = **0** / `pytest` 962 passed, 6 skipped, 7 preexisting failed（gRPC/catalog/jquants の既存失敗、回帰なし）。
- ✅ **Phase A 完了・コミット済** — `chart_viewstate.rs` 新設 (ChartViewState は `base_price_y` + `cell_height` モデル、座標ヘルパ + autoscale 3 system を `RequestAutoscale` event で分離)。chart child に透明 Sprite (alpha 0.001) + `ChartInstrument` + `ChartViewState`。`chart_render.rs` は毎フレーム純 draw に分離。
- ✅ **Phase B 完了・コミット済** — `chart_axes.rs` 新設 (価格軸/時間軸ラベル、`calc_optimal_price_ticks`/`calc_optimal_time_step` 純関数、gutter 子 Text2d を `Changed<ChartViewState>` 駆動で despawn+respawn)。レイアウト定数を `CHART_PANEL_SIZE.y=244` に修正 (title bar 抜けバグ解消)。`set_parent` despawn-race 2 段ガード実装。
- ✅ **Phase C 完了・コミット済** — `chart_interaction.rs` 新設 (左ドラッグ pan + cursor 中心ホイールズーム、`HoverMap` で hover chart 特定)。`pancam_suppression_over_editor_system` を chart draw 領域へ拡張 (Ctrl=キャンバス全体 / 無修飾=chart ズーム)。`cell_height` クランプは px/price-unit (tick_size 比例) で持つ。
- ✅ **Phase D 完了・コミット済** — `chart_crosshair.rs` 新設 (`CrosshairState` + `Pointer<Move>`/`<Out>` observer + 派生 system + 毎フレーム cross line render + price/time readout badge)。observer は cursor だけ保存し派生量は Render 段で計算 (順序非依存)。z: axis +0.3 < cross +0.5 < badge +0.6。
- ✅ **Phase E 完了** — `chart_volume.rs` 新設 (volume サブペイン: 毎フレーム純 draw、Phase A 予約済の volume area に bar、`None` volume skip)。crosshair の volume readout は `chart_crosshair.rs` へ additive 拡張 (`CrosshairState.hovered_volume` + `nearest_candle_volume` 二分探索、price と排他)。polish の **BUY/SELL ボタン削除・single-candle fallback 削除・`TradeButton` enum 削除は A0 で既済**と判明 (Phase E では新規削除無し)。simplify で candle 色を `chart_render.rs` の const に一本化。
- ✅ **Phase F 完了** — `chart_ladder_pane.rs` 新設 (Live モード複合ウィンドウ: Chart 右に bid/ask×10+LAST の Ladder ペインを結合)。`chart_ladder_mode_sync_system` (ExecutionMode 変化 + `Added<WindowRoot>` で Ladder spawn/despawn + 枠リサイズ + chart/price 左シフト)、`ladder_render_system` (per-instrument depth → 21 行、`map.is_changed()` 粗 gate + per-pane `depth_signature` early-out)。**plan ドリフト 2 件を実装時に訂正**（`CHART_CHILD_LOCAL_X_LIVE` 再導出 / chart は WindowRoot の孫）— 下記「Phase F 実装後の確定事実・ドリフト訂正」を参照。

依存: A0 ✅ → A ✅ → {B ✅, C ✅, D ✅} → E ✅ → **F ✅**。検証 baseline（A〜F 込み）: `cargo check` clean / `cargo test --lib` **419 passed, 0 failed**（A〜E の 407 + Phase F の 8 + 並行実装の double-click reset 系 4）。**全 7 フェーズ完了。**

### Phase F 実装後の確定事実・ドリフト訂正（Ladder pane / 将来 polish 着手者へ）

> **⚠️ §F の本文コード片 (`chart_ladder_mode_sync_system` / `ladder_render_system` の擬似コード) は設計時の近似でドリフトしている。実コード `src/ui/chart_ladder_pane.rs` が正。** 以下は実装で確定した訂正。

**ドリフト訂正（§F 本文をこの節で上書きする）:**

1. **chart entity は `WindowRoot` の *孫***（root → content_area → chart）。§F の `chart_ladder_mode_sync_system` 擬似コードは `children_q.get(root_entity)` の直下子を `chart_children` (`With<ChartViewState>`) と照合していたが、chart は content_area の子なので**直下では見つからない**。実コードは root の children のさらに children まで辿り、`ChartViewState` または `PriceDisplay` を持つ孫を見つけてその x をシフトし、その親 (content_area) を Ladder の spawn 先にする。
2. **`CHART_CHILD_LOCAL_X_LIVE` は `-LADDER_WIDTH/2`(=-60) では誤り → `CHART_CHILD_LOCAL_X_REPLAY - LADDER_WIDTH/2`(= **-85**)**。Phase B 以降 Replay の chart 中心は 0 ではなく `-25` (右 50px を price gutter に空けるため)。Live で枠が左へ `LADDER_WIDTH/2`(60) 広がる分を追従させないと chart+gutter が左端 flush にならない。`-60` のままだと price gutter `[95,145]` が ladder `[120,240]` と重なる。`chart_viewstate.rs` の定数を修正済。§F mode_sync 擬似コードの `else { 0.0 }` も誤りで、実コードは `CHART_CHILD_LOCAL_X_REPLAY`(-25)。
3. **Ladder pane は `content_area` の子**（§F 擬似コードの `.set_parent(root_entity)` は誤り）。root 直下に置くと ① title bar drag observer に bubble して ladder ドラッグで窓が動く、② title bar 領域 (上 40px) に被る。content_area (root-local `(0, -title_bar_half)`) の子にすると title bar 下に収まり、高さは `CHART_PANEL_SIZE.y - TITLE_BAR_HEIGHT`(=204) でちょうど content 領域に一致。x は content_area の x=0 なので `LADDER_PANE_LOCAL_X`(180) のまま枠右端 flush。**結果として §F Caveat #35 (LadderPane に Pointer observer + propagate(false)) は不要**になった（content_area 子なので title bar observer に届かない。close button が root 直下に置かれて drag bubble を断つのと同じ仕組み）。
4. **行の despawn は `despawn_recursive`**（§F 擬似コードの `commands.entity(child).despawn()` は誤り、Caveat #26）。各行は背景 Sprite + Text2d 子の 2 entity 構成なので、`despawn()` だと Text2d 子が orphan になる。
5. **`DepthSnapshot` は `timestamp_ms: Option<i64>` も持つ**（trading.rs 実体）。Ladder UI では未使用だが signature には含めない（時刻 tick で無駄 rebuild しないため price/size のみ畳む）。

**設計思想の確定（Phase F で load-bearing と判明）:**

- **per-pane `depth_signature` early-out が必須**（simplify レビュー指摘）。§F 本文は「`map.is_changed()` 粗 gate で十分」としていたが、`InstrumentTradingDataMap.is_changed()` は**他銘柄の OHLC tick でも立つ**粗 flag。これだけだと depth 不変の pane も毎 tick 21 行 (×2 entity = 42) を despawn+respawn する。`LadderPane.last_depth_signature: u64` に depth+last を畳んで保持し、不変なら rebuild を skip する（chart_viewstate の `last_seen_ohlc_signature` と同じ per-entity early-out パターン）。粗 gate (`map.is_changed() || Added<LadderPane>`) → per-pane signature の 2 段。
- **責務 2 層 = mode_sync (構造) / render (中身)**。mode_sync は ExecutionMode 駆動で「Ladder の有無・枠サイズ・chart 位置」という*構造*だけ reconcile（gated、稀）。render は depth 駆動で*中身*（行）だけ更新。observer は不要（Ladder は表示専用）。
- **`Ref<LadderPane>::is_added()` で新規 pane 判定**（§F 本文通り。`Has<Added<T>>` は compile error）。render は `Mut<LadderPane>` を取って signature を書き戻すので query は `(Entity, Mut<LadderPane>, Option<&Children>)`。

**Tips（将来 polish 着手者向け）:**

- **既知の見た目の限界**: 枠リサイズは Caveat #34 通り root の `Sprite.custom_size` のみ。title bar / inner glow / rim の各 Sprite は `spawn_floating_window` 時の 360px 幅のままなので、Live では**枠だけ 480px に広がり title bar が枠より狭く見える**。polish するなら mode_sync で title_bar (`With<TitleBar>`) / inner glow / rim / close button 位置も追従リサイズする必要があるが、inner glow/rim は marker 無しなので `spawn_floating_window` 側に marker 追加か、content_area に `ContentArea` marker を足して traversal を直接 query 化するのが先。
- **Autoscale toggle button（§Polish の未実装 TODO）**: 並行実装で **double-click reset** が landing 済 (`chart_interaction.rs::install_chart_autoscale_reset_observer` + `ChartClickState` + `ChartViewState::reset_view()`)。chart 本体ダブルクリックで pan/zoom リセット + autoscale 再有効化。隅の小ボタンは未実装のまま。
- **検算テスト** `live_layout_fits_frame_without_overlap` が chart`[-240,70]`/gutter`[70,120]`/ladder`[120,240]` の枠`[-240,240]` flush を assert。レイアウト定数を触ったらこれが回帰検出する。
...
### A〜D 実装後の確定事実・ドリフト訂正・Tips（E/F 着手者へ最優先で読む）

> **⚠️ 計画書本文のコード片は「設計時の近似」。A〜D は実装で確定したので、座標式・型・system 名は必ず実コードを Read で確認すること。** 以下は本文のどこがドリフトしているかと、E/F が前提にできる確定事実。

**ドリフト訂正（本文の該当箇所はこの節で上書きする）:**

1. **`price_to_y` / `y_to_price` の実式**（本文 §Phase E L1004〜の `(price-base) * cell_height` は drift）。実コード（`chart_viewstate.rs`）は **tick 単位**で持つ:
   ```rust
   // ticks = (price - base_price_y) / tick_size。main_area_center_y() を 0 オフセットにする。
   price_to_y(p) = main_area_center_y() + ((p - base_price_y) / tick_size) * cell_height * scaling + translation.y
   // main_area_center_y() = main_area_y_bottom() + main_area_height() / 2.0
   ```
   `cell_height` の意味は「**1 tick あたり px**」（"1 price unit あたり px" ではない）。autoscale 出力は `cell_height = main_area_height * tick_size / range`。Phase C の zoom クランプが px/price-unit (`cell_height/tick_size`) で持つのはこのため。`tick_size`/`decimals` は現状 hardcode (`0.01` / `2`)。
2. **`DepthLevel` は `{ price: f64, size: f64 }`**（本文 L100-104・L285 の `qty: u64` は drift。前提 2 と同じ。`#[serde(default)]` 付き）。
3. **`InstrumentTradingData` は slim**（前提 3）。本文 §2-Resource 分割 L80-92 の太った定義は **採用していない**。実体は `{ price: Option<f64>, ohlc_points, depth }` のみ。
4. **`OhlcPoint` は `timestamp_ms` と `open_time_ms` の両方を持つ**。chart の X 軸基準は `open_time_ms`。`volume: Option<f32>`（`#[serde(default)]`）。
5. **`TradingSession` は `timer: Timer` フィールドを持つ**（本文 L68-75 の定義に加えて poll 用 `Timer::from_seconds(0.5, Repeating)`）。

**設計思想の確定（A〜D で load-bearing と判明、E/F も踏襲する）:**

- **責務 3 層 = observer / derive / render**。observer (`Pointer<*>`、schedule 外) は生入力 (cursor 等) だけ保存し、autoscale 確定値 (`base_price_y`/`cell_height`) を**読まない**（Caveat #28: observer の発火順は `ChartSet::Autoscale` 前後どちらか不定）。派生量は Render 段の regular system で計算。Phase E の `hovered_volume` も同じ分け方で additive 拡張する。
- **immediate-mode 描画は毎フレーム、Changed gate 禁止**（Caveat #11）。`chart_render` / `chart_crosshair_render` / 今後の `volume_render` は `ShapePainter` で毎フレーム発行し、描画スキップは per-entity `continue` で行う。retained Text2d（axis label / crosshair badge）だけ `Changed<...>` 駆動で despawn+respawn してよい。
- **self-`Changed` ループは DerefMut 抑制ガードで断つ**（Caveat #29）。`if old != new { old = new; }` で同値代入を避ける。autoscale 3 system / crosshair derive で実証済（spawn 後 5 frame で `Changed=0` の収束テストを必ず添える）。
- **gutter 子の動的 spawn は set_parent despawn-race 対策 2 段が必須**（Caveat #26）。① `live_*_gutter.contains(ref)` 生存ガード + ② 動的 spawn system を `.after(instrument_chart_sync_system)`。chart panel は prune→sync で spawn 直後 despawn される startup churn があるため。axis label / crosshair badge で実装済、volume badge も同様に。
- **`despawn` は子孫を消さない**（Caveat #26）。背景 Sprite + Text2d 子のような複合 badge は `despawn_recursive` で消す（crosshair badge が実例）。

**Tips（E 着手者向け）:**

- volume area は **Phase A で予約済み**（`volume_area_height()`=bounds.y*0.2 / `main_area_y_bottom()`）。`price_to_y` は volume area を除いた `main_area_center_y()` 基準なので、**E は純 additive**（座標式・round-trip テストを触らない）。
- `visible_candle_slice()` を volume 集計でも再利用する（autoscale と同一スライス）。
- crosshair の volume readout は `CrosshairState` に `hovered_volume: Option<f32>` を**追加するだけ**（`#[derive(Default)]` で `None`、Out observer は touch 不要）。`chart_crosshair_derive_system` の query を `(&ChartViewState, &ChartInstrument, &mut CrosshairState)` + `Res<InstrumentTradingDataMap>` に広げ、`c.y < main_area_y_bottom()` のときだけ最近傍 candle の volume を入れる。price badge と volume badge は排他（同時に Some にならない）。`crosshair_badge_system` に volume 分岐を足す（既存 `spawn_badge` ヘルパを再利用）。
- 新 system は `ChartSet::Render` に入れる。`add_systems` タプルは現状 chart ブロックで 12 要素（20 上限まで余裕、Caveat #15）。
- polish の BUY/SELL ボタン削除は `grep TradeButton::` で他参照ゼロを確認してから enum ごと消す（売買 UI は Phase 9 で別ウィンドウ新設）。
...
> **⚠️ ドリフト訂正（A0 で確定した実装が正）**: 下の太った定義は採用していない。実体は slim な
> `{ price: Option<f64>, ohlc_points: Vec<OhlcPoint>, depth: Option<DepthSnapshot> }`（全 field `#[serde(default)]`）。
> `history_points/open/high/low/close/open_time_ms` は **持たない**（wire が送らない）。chart は `ohlc_points.last().close` を使う。

```rust
// ⚠️ これは設計時の旧案。実コード src/trading.rs は上記 slim 版。
...
// ⚠️ ドリフト訂正: 実コードは `{ price: f64, size: f64 }`（key は "size"・型は f64、NOT qty/u64）。
//    Python live/adapter.py の board に合わせた。Copy ではなく Clone + Default + serde。
#[derive(Debug, Clone, Default)]
...
> **✅ 実装後の確定値 (この設計ブロックの数値ドリフトを上書き)**: 下記「設計決定」「ExecutionMode 切替時の動作」の擬似コードには `Replay の chart x = 0` / `Live = -LADDER_WIDTH/2 (-60)` という記載があるが**どちらも誤り**。Phase B 以降 Replay の chart 中心は `CHART_CHILD_LOCAL_X_REPLAY = -25` (右 50px を price gutter に空ける)、Live は `CHART_CHILD_LOCAL_X_LIVE = CHART_CHILD_LOCAL_X_REPLAY - LADDER_WIDTH/2 = -85`。また `LadderPane` は WindowRoot 直下ではなく **content_area の子** (title bar drag bubble 回避 + title bar 下に収める)。price text (PriceDisplay) も chart と同量シフトする。詳細は冒頭「Phase F 実装後の確定事実・ドリフト訂正」。
...
- `src/trading.rs` (depth 分も A0):
  - `InstrumentTradingData` に `depth: Option<DepthSnapshot>` を追加。`DepthSnapshot { bids: Vec<DepthLevel>, asks: Vec<DepthLevel> }` / `DepthLevel { price: f64, size: f64 }` を Rust 型として定義 (`models.py` の Pydantic 契約と field 名一致、`#[serde(default)]`。⚠️ 旧案の `qty: u64` ではなく `size: f64` で確定済)
...
> ## ✅ Phase E 実装記録（2026-05-20、未コミット — レビュー待ち）
>
> **検証**: `cargo check` clean / `cargo test --lib` **407 passed, 0 failed**（A〜D の 397 + Phase E 新規 10）。**目視 E2E は未実施**（座標式・None skip・crosshair volume 分岐は単体テスト済、実画面の volume bar 見た目は要確認）。
>
> **実装したもの:**
> 1. **新規 `src/ui/chart_volume.rs`** — `volume_render_system`（`ChartSet::Render`、毎フレーム純 draw、`Changed` gate しない＝Caveat #11）。Phase A 予約済の volume area（draw 下端 `-bounds.y/2` 〜 `volume_area_height()`）に bar を描画。純関数 `max_visible_volume` / `volume_bar_height`（`[0,1]` クランプ）/ `format_volume`（K/M/B 略記、badge 用）も同居。
> 2. **`chart_crosshair.rs` の additive 拡張**（計画通り）— `CrosshairState.hovered_volume: Option<f32>` 追加（Out observer でリセット）、`chart_crosshair_derive_system` に `Res<InstrumentTradingDataMap>` + `&ChartInstrument` を追加して volume area のときだけ最近傍 candle の volume を `nearest_candle_volume`（二分探索）で計算（`hovered_price` と排他）、`crosshair_badge_system` に volume badge 分岐（price gutter の cursor.y 行）。
> 3. **`mod.rs`** — `chart_volume` module 登録 + `volume_render_system.in_set(ChartSet::Render)`（chart タプル 12→13、Caveat #15 の 20 上限内）。
>
> **⚠️ 計画書コード片のドリフト訂正（実コードが正）:**
> - **volume bar 幅は `state.body_half_width() * 2.0`**（下の旧 snippet L1094 の `state.cell_width * 0.8` は drift）。candle body と同じ式にして zoom 追随＋candle と縦に揃える。`body_half_width()` は `scaling` を畳み込み済。
> - **bar 色は `chart_render.rs::BULLISH_CANDLE_COLOR` / `BEARISH_CANDLE_COLOR`（pub const）を `with_alpha(0.6)` で派生**（下の旧 snippet L1084-1087 の `Color::srgba(...)` ハードコードは drift）。simplify レビューで candle 色リテラルが 3 重化していたため `chart_render.rs` に const を新設して single source 化。**Phase F の Ladder で bid/ask 色を足すときも同様に const を一本化すること**。
> - **`hovered_volume` の埋め方は「cursor.x → `x_to_time_ms` → `nearest_candle_volume` 二分探索」**。当該 candle の `volume` が `None` ならそのまま `None`（badge を出さない）。
>
> **polish 項目の確定状況（重要 — 着手前に把握）:**
> - ✅ **BUY/SELL ボタン削除 / `TradeButton` enum 削除 / `button.rs` 削除 / `systems.rs::button_system` 削除 は A0 で既済**。Phase E では新規削除作業は無かった（`rg 'TradeButton'` は plan/archive doc のみヒット、`src/ui/button.rs` は存在しない）。本書 L394 の A0 削除リストと整合。**売買 UI は Phase 9 で別ウィンドウ新設**（chart には同居させない）。
> - ✅ **single-candle fallback 削除も Phase A 分割時に既済**。現 `chart_render.rs` は `ohlc.is_empty()` / `visible.is_empty()` で per-entity `continue` するだけで fallback 描画は無い。
>
> **Phase F 着手者への申し送り:**
> - volume render と crosshair の責務 3 層（observer / derive / render）は踏襲済。Ladder も「板スナップショットを read-only に取り毎フレーム純 draw、retained 行は `Changed` 駆動」で同じ分け方にする。
> - `InstrumentTradingData.depth: Option<DepthSnapshot>`（A0 で surface 済、`DepthLevel{price:f64, size:f64}`）を per-instrument lookup する。chart の OHLC と同じく `ChartInstrument.instrument_id` で引く。
...
**`cell_height` の意味は Phase A で確定**: 「**main area 内の 1 tick あたり px**」(volume area の高さは含まない)。`price_to_y` / `y_to_price` は `main_area_height()` を境界とする。

> **⚠️ ドリフト訂正（実コードが正）**: 下の旧 snippet は `(price-base) * cell_height` だが、実コード（`chart_viewstate.rs`）は **tick 単位**で `((price-base)/tick_size) * cell_height` を使い、0 オフセットは `main_area_y_bottom` ではなく `main_area_center_y()`（= `main_area_y_bottom() + main_area_height()/2.0`）。冒頭「A〜D 実装後の確定事実」節 1 を参照。

```rust
// ⚠️ 設計時の旧案。実コードは tick 単位 + main_area_center_y 基準（上記訂正を参照）。
...
- ⬜ **Autoscale toggle button — 未実装（Phase E スコープ外として保留）**。現状 `auto_scale` は default `true` で spawn し pan/zoom 開始で `false` になるが、**手動で再 enable する UI が無い**。Phase F または別 polish PR で chart 隅に小ボタン（`auto_scale = !auto_scale`）を足すのが TODO。
- ✅ **crosshair の volume readout — 実装済**（`chart_crosshair.rs` へ additive 拡張、計画通り）:
  1. ✅ `CrosshairState` に `pub hovered_volume: Option<f32>` を追加（`#[derive(Default)]` で `None`）。**Out observer も `hovered_volume = None` でリセット**した（計画は「touch 不要」だったが stale 防止のため明示リセットに変更。badge は cursor_world ガードがあるので機能上は等価だが、状態を clean に保つ方が安全）。
  2. ✅ `chart_crosshair_derive_system` の query を `(&ChartViewState, &ChartInstrument, &mut CrosshairState)` + `Res<InstrumentTradingDataMap>` に拡張。**既存 derive テスト 4 本に `app.init_resource::<InstrumentTradingDataMap>()` を追加が必要**（bevy-engine: 既存 system に `Res` を足すとその system を踏む全テスト App で init 漏れ panic する罠）。
  3. ✅ volume area（`c.y < main_area_y_bottom()`）のみ `nearest_candle_volume`（私有 fn、`binary_search_by_key(open_time_ms)` + 挿入位置の前後距離比較）で最近傍 candle の volume を返す。当該 candle の `volume` が `None` ならそのまま `None`。main area 内なら `None`。
  4. ✅ DerefMut 抑制ガード（Caveat #29）を `hovered_volume` にも適用。
  5. ✅ badge は `hovered_price`/`hovered_volume` 排他で別 entity spawn（既存 `spawn_badge` ヘルパ再利用）。volume badge は **price gutter の cursor.y 行**に出す（price badge と対称＝horizontal cross line 上、`format_volume` で K/M/B 略記）。コードは `Option::zip` で 2 段ネストに整理。
- ✅ **single-candle fallback 削除 — Phase A 分割時に既済**。現 `chart_render.rs` は `ohlc.is_empty()` / `visible.is_empty()` で per-entity `continue` するだけで fallback 描画は無い（Phase E で新規削除作業無し）。
- ✅ **旧 chart.rs テスト群の `HistoryPoint`→`InstrumentTradingData` 書き換え — A 系で既済**。現 chart 系テストは `OhlcPoint`/`InstrumentTradingData` 経由で記述済。
- ✅ **BUY/SELL ボタン削除 / `TradeButton` enum 削除 — A0 で既済**（本書 L394 の A0 削除リスト参照）。`rg 'TradeButton'` は plan/archive doc のみヒット、`src/ui/button.rs` も `systems.rs::button_system` も存在しない。Phase E では新規削除作業は無かった。売買入力は [Phase 9 - Live Account and Order API](docs/plan/Phase%209%20-%20Live%20Account%20and%20Order%20API.md) で**独立した「売買入力ウィンドウ」として新設**される（chart panel に同居させない）。

### ✅ Phase F: Live モード複合ウィンドウ (ローソク足＋Ladder 結合) — 完了 (2026-05-20)

> **⚠️ 以下の擬似コードは設計時の近似でドリフトしている。実コード `src/ui/chart_ladder_pane.rs` が正。**
> 主な訂正は上記「Phase F 実装後の確定事実・ドリフト訂正」を参照 (chart は WindowRoot の*孫* /
> `CHART_CHILD_LOCAL_X_LIVE`=-85 / Ladder は content_area の子 / 行 despawn は recursive /
> per-pane signature early-out を追加 / Caveat #35 は不要化)。
...
**Verification (Phase F):** ✅ = `cargo test --lib chart_ladder_pane` (8 本) でカバー済 / 🖐 = 実機 E2E 未実施 (要 backend + ライブ depth フィード)

1. ✅ `ExecutionMode` を Replay → Live に切替えると Chart ウィンドウ右に Ladder ペインが出現し、WindowRoot が広がる。**chart+gutter が左に寄り (chart child x = `CHART_CHILD_LOCAL_X_LIVE` = -85、※plan 旧記載の -60 は誤り)、Ladder が枠内 `[120,240]` に収まる** (枠右端 240 をはみ出さない) — `mode_sync_spawns_ladder_in_live_and_despawns_in_replay` + `live_layout_fits_frame_without_overlap`
2. 🖐 Live モードで `MockVenueAdapter` の `emit_depth_snapshot()` を呼ぶと、**該当 instrument の Ladder のみ** bid/ask 数値が更新される (他銘柄の chart の Ladder は変わらない = per-instrument 分離) — unit では `ladder_render_is_per_instrument` で代替検証済、実機フィードは未確認
3. ✅ **複数銘柄を Live 購読し各々に depth を流すと、各 chart の Ladder が独立に自分の板を描く** (single-global 退行が無い) — `ladder_render_is_per_instrument`
4. 🖐 **ローソク足軌跡**: Live で trade inject → live aggregation 経由で `per_instrument[id].ohlc_points` の 1 分足更新 → ローソク足が伸びる (Phase 8 live_runner aggregator 依存、Phase F 範囲外。実機未確認)
5. ✅ `ExecutionMode` を Live → Replay に戻すと Ladder が消え、ウィンドウがコンパクトサイズに戻り、chart child x が `CHART_CHILD_LOCAL_X_REPLAY` (-25、※plan 旧記載の 0 は誤り) に戻る — 同上 mode_sync テスト
6. ✅ `depth == None` (Live 接続前 / 未購読) の状態では「板情報なし」プレースホルダが表示される — `ladder_render_placeholder_when_no_depth`
7. 🖐 Phase E の Volume サブペイン / Phase D の Crosshair が Ladder 追加後も正常動作する (非退行) — Ladder は別 entity ツリーで干渉しない設計だが実機目視は未実施
8. ✅→不要 Ladder の Drag bubble — content_area 子にしたため title bar observer に届かず、観測子・`propagate(false)` 自体が不要 (Caveat #35 の訂正を参照)
9. ✅ depth 不変フレームでは行を再生成しない (per-pane signature early-out) — `ladder_skips_rebuild_when_depth_unchanged` / 変化時は 21 行に置換 = `ladder_rows_replace_not_accumulate`

> 🖐 項目 (実機 E2E) は **稼働中の python backend + live venue depth フィードが必要**で本実装セッションでは未実施。コードレベル (型・座標・spawn/despawn・per-instrument 分離・signature) は unit でカバー済。実機検証は `e2e-testing` スキルで別途。
...
- ✅ `src/ui/chart_ladder_pane.rs` (Phase F) — `LadderPane{chart_root, last_depth_signature}` / `LadderRow{kind, index}` / `LadderRowKind{Ask,Last,Bid}` Component、`chart_ladder_mode_sync_system` (ExecutionMode 変化 + `Added<WindowRoot>` で Ladder spawn/despawn + WindowRoot リサイズ + chart/price 左シフト)、`ladder_render_system` (per-instrument depth → Text2d 行生成、`map.is_changed()` 粗 gate + per-pane `depth_signature` early-out)、純関数 `ladder_row_y` / `format_level_label` / `depth_signature`。**ドリフト訂正**: chart は WindowRoot の孫なので mode_sync は孫まで traverse / `CHART_CHILD_LOCAL_X_LIVE`=-85 / Ladder は content_area の子 / 行 despawn は recursive (上記「Phase F 実装後の確定事実」参照)
...
34. ✅ **Phase F: `Sprite.custom_size` でリサイズ、`Transform.scale` は使わない** — WindowRoot の scale を変えると chart 子 entity のローソク足ピクセルサイズも変わる。ウィンドウ枠の拡張は必ず `root_sprite.custom_size = Some(LIVE_COMBINED_PANEL_SIZE)` で行う。**実装後追記**: root Sprite のみリサイズしたので title bar / inner glow / rim は 360px 幅のまま残る既知の見た目限界あり (上記「Tips」参照)
35. ⚠️→不要 **Phase F: LadderPane の Drag bubble** — **実装で不要化**。Ladder pane を root 直下ではなく **content_area の子**にしたため、title bar drag observer (title bar entity に付く) には届かず bubble しない (close button が root 直下に置かれる仕組みと同じ)。observer + `propagate(false)` は付けていない。もし将来 root 直下に置き換えるなら本 Caveat が再び有効になる
