...
6.  chart_crosshair_render_system    (毎フレーム、ShapePainter で別 z。cursor_world==None で continue)  [Phase D]
...
- **既存 `Res<TradingData>` reader の移行 — A0 同一 PR で全 8 箇所を完了する**:
...
- 全 reader 移行完了後、`Res<TradingData>` が **0 箇所参照**になっていることを `grep -n 'Res<TradingData>' src/` で確認した上で `TradingData` 構造体を A0 同一 PR で削除する。**legacy alias は残さない**

**Phase A0 確定スコープ (重要)**: 「new resource 追加」+「`backend_update_system` 書き換え」+「全 reader 移行」+「`TradingData` 削除」+「proto/nautilus volume 集計」を**全て同一 PR**で landing する。**「legacy 経路を残してフェーズ間で並走する」設計は採用しない** (per-instrument 経路と single resource 経路が二重に存在すると `price_simulation_system` ↔ `backend_update_system` の write 順依存が二系統で発生し、後続フェーズの bug 表面化が遅延するため)。
...
**Verification:**
- `cargo check` + `cargo test --lib` (`Res<TradingData>` の grep 結果 0 件を含む)
...
fn chart_viewstate_update_system(
    ...
    // (a) data 変動経路: map が変わったときだけ「新規 / OHLC 件数増」を反映
    mut data_q: Query<(&ChartInstrument, &mut ChartViewState)>,
    // (b) interaction 経路: pan/zoom などで ChartViewState 自体が触られたとき autoscale 派生量を更新
    mut interaction_q: Query<
        (&ChartInstrument, &mut ChartViewState),
        (Changed<ChartViewState>, Without<JustHandled>),  // Without は擬似、実装では同一 system 内 flag で十分
    >,
) {
    // (a) data 変動
    if map.is_changed() {
        for (chart_instrument, mut state) in &mut data_q {
            let Some(data) = map.map.get(&chart_instrument.instrument_id) else { continue };
            // 新規 entity も last_seen_ohlc_len=0 で初期化されているので「件数変化」だけで判定可。
            // bevy 0.15 では Added<ChartViewState> を別 query で重ねるか、初期値 usize::MAX で初回必ず通す方式でもよい。
            if data.ohlc_points.len() == state.last_seen_ohlc_len { continue; }
            ...
            if state.auto_scale {
                ...
                // state.base_price_y / state.cell_height を更新
            }
        }
    }
    // (b) ChartViewState 自体の変動 (pan/zoom など)
    //     autoscale=false なら派生量更新不要だが、cell_width/translation 変動で
    //     可視 candle range が変わるので autoscale=true のときは再計算する。
    for (chart_instrument, mut state) in &mut interaction_q {
        let Some(data) = map.map.get(&chart_instrument.instrument_id) else { continue };
        if !state.auto_scale { continue; }
        // autoscale 再計算 (上の (a) と同じロジックを呼ぶ — 別 fn に切り出して両方から call)
        let _ = data;
    }
}
```

⚠️ **実装上の罠**: 上の擬似コードで `interaction_q` を `&mut ChartViewState` + `Changed<ChartViewState>` で取ると、自分自身の書き込みが次フレーム以降 `Changed` を立て直して無限ループに見える挙動になる。実装時は (1) 同一 system 内で `(a)` と `(b)` の書き込み対象を区別する local flag、または (2) `Changed<ChartViewState>` クエリで読みだけ行い `base_price_y`/`cell_height` の new value を `Commands` 経由で次フレームに反映、のいずれかにする。Phase A 着手時に bevy-engine スキルで「self-`Changed` ループの避け方」を確認。

⚠️ **`state.is_added()` は使わない**: `&mut ChartViewState` には `is_added()` API が無い (`Mut<T>` 経由でも `is_changed()` のみ。`is_added()` を取りたければ `Query<..., Added<ChartViewState>>` を別 query として持つ)。初回更新を確実に走らせたい場合は `ChartViewState::default()` の `last_seen_ohlc_len` を `usize::MAX` (sentinel) で初期化し、最初の data 比較で必ず差分が立つようにする。
...
fn price_axis_labels_system(
    ...
    for (chart_entity, state, gutter_ref) in &chart_q {
        // 既存ラベルを despawn (PriceGutter 配下の子なので親 despawn でも可だが、ラベル単位の
        // 出し入れに留めることで gutter entity 自体の transform を温存)
...
`ChartViewState.bounds` には `CHART_DRAW_SIZE` を入れる (axis label 領域を含まない)。`spawn_chart_panel` で Y gutter / X gutter の空 child entity (`PriceGutter` / `TimeGutter` マーカー Component 付き、Transform は chart-local) を準備し、その Entity id を chart entity 側に `PriceGutterRef(Entity)` / `TimeGutterRef(Entity)` Component として埋め込む。axis label system は前述の通り `.set_parent(gutter_ref.0)` でラベルを gutter 子として spawn する。
...
⚠️ **`EventReader<MouseWheel>` ではなく `Pointer<Scroll>` observer を使う**。グローバル `EventReader<MouseWheel>` だと「複数 chart のどれにカーソルがあるか」を `camera.viewport_to_world_2d` + sprite bounds で逆引きする必要があり、UI 全体の z 順 / picking 結果との二重実装になる。Bevy 0.15 picking backend は scroll event を `Pointer<Scroll>` として正しい entity に dispatch するので、chart sprite に attach した observer で受ければ「どの chart か」と「local cursor 位置」が両方ゼロコストで取れる。floating_window.rs:54-150 の `Pointer<Down>` / `Pointer<Drag>` と同じパターン。
...
fn install_chart_scroll_observer(
    mut commands: Commands,
    new_charts: Query<Entity, (Added<ChartViewState>, With<Sprite>)>,
) {
    for entity in &new_charts {
        commands.entity(entity).observe(
            |trigger: Trigger<Pointer<Scroll>>,
             mut chart_q: Query<(&GlobalTransform, &mut ChartViewState)>| {
                let Ok((gt, mut state)) = chart_q.get_mut(trigger.entity()) else { return };
                let hit = trigger.event().hit.position.unwrap_or(Vec3::ZERO);
                let cursor_local = (hit - gt.translation()).xy();
                // ズーム前の cursor 指す価格 / 時刻
                let cursor_price = state.y_to_price(cursor_local.y);
                let cursor_time  = state.x_to_time_ms(cursor_local.x);
                let factor = 1.0 + trigger.event().y / ZOOM_SENSITIVITY;
                ...
                // ズーム後に cursor 位置が同じ価格 / 時刻を指すよう translation 補正
...
⚠️ **`Pointer<Scroll>` の `event().y` 符号**: Bevy 0.15 では up scroll が `+y`、down scroll が `-y` (line-based、`unit == MouseScrollUnit::Line`)。OS / マウス / トラックパッド により `unit == Pixel` で値が桁違いに来るケースがあるので、Phase C で `match event.unit { Line => y, Pixel => y / 20.0 }` のような正規化ガードを入れる (flowsurface 同等の感度を出すため)。
...
- `src/ui/systems.rs` (Phase A0) — `update_price_display` を `Res<LastPrices>` の `SelectedSymbol` lookup に書き換え (旧 `data.price` は tick 価格なので `InstrumentTradingDataMap` ではなく `LastPrices` が正しい移行先。M4 修正の `last_price` 削除と整合)
