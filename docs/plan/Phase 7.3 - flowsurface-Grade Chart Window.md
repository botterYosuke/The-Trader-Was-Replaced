...
- ⬜ Phase F — Live モード複合ウィンドウ (ローソク足＋Ladder)（次の作業）

依存: A0 ✅ → A ✅ → {B ✅, C ✅, D ✅} → E ✅ → **F**。検証 baseline（A〜E 込み）: `cargo check` clean / `cargo test --lib` **407 passed, 0 failed**（A〜D の 397 + Phase E の 10）。
