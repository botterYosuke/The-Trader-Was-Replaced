...
### 手動検証 Tips（engine_runner.run 接続後）
...
起動直後のログが `Engine initialized in paused state.` ならOK。
`Starting DataEngine core` だけ出て止まった場合はポート 19876 が別プロセスに占有されている（次項参照）。

**起動時 RUNNING 問題と対処**

`serve(auto_start=True)` のため、backend は `replay_state=RUNNING` で立ち上がることがある。
この状態で Rust UI から Run すると LoadReplayData が `INVALID_STATE` で失敗する。
...
ch = grpc.insecure_channel('localhost:19876')
st = engine_pb2_grpc.DataEngineStub(ch)
d = json.loads(st.GetState(engine_pb2.GetStateRequest(token='testtoken'), timeout=3).json_data)
print('replay_state:', d.get('replay_state'))
"
```
...
ch = grpc.insecure_channel('localhost:19876')
st = engine_pb2_grpc.DataEngineStub(ch)
r = st.ForceStopReplay(engine_pb2.ForceStopReplayRequest(token='testtoken', request_id='reset'), timeout=5)
print('ForceStop:', r.success, engine_pb2.EngineState.Name(r.current_state))
"
```
...
1. `Open Strategy...` → `python/tests/data/test_strategy_daily.py` を選択
2. Run ボタンを押す

**Python ログで確認するポイント（この順で出ること）**

```text
LoadReplayData success=True state=LOADED
StartEngine: strategy_file='...cache...'
...
StartEngine: run complete run_id=<ts>-test_strategy_daily-1301_TSE run_dir=... summary=...
...
```powershell
ls "$env:APPDATA\flowsurface\run-buffer"
ls "$env:APPDATA\flowsurface\run-buffer\<run_id>"
# → meta.json  fills.jsonl  equity.jsonl  summary.json の4ファイル
```

**ポート 19876 占有プロセスの強制停止**（必要な場合）

```powershell
$pid = (Get-NetTCPConnection -LocalPort 19876 -ErrorAction SilentlyContinue).OwningProcess
