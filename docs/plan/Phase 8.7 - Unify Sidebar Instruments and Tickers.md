...
**v4 修正: bare path 形式を正規対応** — 既存 fixture ([e2e_instruments_ref_locked.json](../../python/tests/data/e2e_instruments_ref_locked.json), [e2e_instruments_ref_mixed_locked.json](../../python/tests/data/e2e_instruments_ref_mixed_locked.json)) は `"refs/universe_topix_core.json"` のような **JSON pointer 無しの bare path**。Phase 8.7 では既存形式を壊さず、JSON pointer は optional 拡張として後置:
...
- `e2e_instruments_ref_locked.json`: bare path、root が list[str]。**ただし target file `python/tests/data/refs/universe_topix_core.json` が repo に存在しない**（v5.2 レビュー Claim 1）。Phase 8.7 で resolver を fail-closed 化すると、現状の fixture は load 時に `ScenarioValidationError` を確実に raise する。**修正アクション**: `python/tests/data/refs/` ディレクトリを新設し `universe_topix_core.json`（中身: `["1301.TSE", "7203.TSE"]` 程度の list[str]）を commit する。または fixture 側を inline `instruments` に倒して `instruments_ref` 経路は新規 fixture でのみ検証する
