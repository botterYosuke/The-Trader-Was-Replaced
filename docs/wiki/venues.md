...
| 状態 | 意味 | 色 |
|---|---|---|
| DISCONNECTED | 未接続 | グレー |
| AUTHENTICATING | ログイン処理中 | 黄 |
| CONNECTED | ログイン完了 | 水色 |
| SUBSCRIBED | 銘柄購読中 | 緑 |
| RECONNECTING | 再接続中 | 黄 |
| ERROR | エラー | 赤 |
...
- サイドバーの **Instruments** セクションで銘柄を追加・選択します。
- Live モード（Manual / Auto）で銘柄行をクリックすると、その銘柄の市場データ購読（SubscribeMarketData）が発火し、状態が SUBSCRIBED に進みます。
- 各銘柄行には最新価格が表示されます（Replay / Live 共通）。
