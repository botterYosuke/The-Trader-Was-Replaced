...
Order は LiveManual 限定のサイドバー **Order** ボタンから開くワールド空間のスプライト製フローティングウィンドウ（タイトル `ORDER`）で、ドラッグ移動・パン追随・`×` で閉じる操作に対応し、LiveManual を離れると despawn します（レイアウト永続化の対象外）。 [K10]。Live Run Panel は実行モードに応じて表示される操作系パネル（Bevy UI Node + Interaction）で、Panels ボタンではなく起動時に配置されモードで表示が切り替わります。各パネルの詳細は [ウィンドウとパネル](windows-and-panels.md)、チャートの内容は [チャート](chart.md) を参照してください。
