...
| [フッター](#フッター) | 下 | 実行モードトグル、再生コントロール、速度、Venue 状態、backend 状態 [I4]/[A1]/[A2]/[A3]/[A4]/[G1] |
...
| backend ステータス | `backend: DISABLED` / `OK` / `ERR` / `...` [G1]/[G2]/[G3] |

**`|<`** / **`<`** / **`>`** / **`■`** と速度セレクタは **Replay** のときだけ表示されます（Manual / Auto では非表示）。**▶** は **Auto** でも表示され、起動・Pause/Resume を担います（Running → **`||`** 一時停止、Paused → **`▶`** 再開 [N13]）。実行モードの違いは [実行モード](modes.md)、再生操作の詳細は [Replay 実行](replay.md) を参照してください。 [I4]/[N5]
