...
- **現状（issue #40）**: 専用の **「Promote to Live」ボタン**と **Safety Rails モーダル**は撤去済みで、UI からの Live Auto 起動入口は一旦無い。起動経路はフッターの transport（再生）ボタンへ再配線する予定（Auto に切替 → `[File]▸[Open]` で戦略読み込み → **▶** で起動）。Live 実行エンジンと起動 RPC（`StartLiveStrategy` 等）は温存。起動経路と Safety Rails は [strategy.md](strategy.md)。
