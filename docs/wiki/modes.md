...
- サイドバーに **Startup** パネル（開始日・終了日・粒度・初期資金）が表示される。 [J7]/[J8]
- 操作フローの詳細は [replay.md](replay.md)。

### Manual（ライブ・手動発注）

- **ライブ市場**に接続し、ユーザーが OrderPanel（発注フォーム）から**手動で発注**する。新規・訂正・取消、2 段階 confirm、第二暗証番号モーダル（Tachibana）を備える。詳細は [orders.md](orders.md)。 [K7]/[K9]/[K10]/[K12]
- トランスポート操作・再生速度・Startup パネルは非表示になる（過去データ再生の概念がないため）。 [I4]

### Auto（ライブ・戦略自動発注）

- **ライブ市場**に接続し、戦略が**自動で発注**する。Replay で検証した戦略を **Promote to Live** で昇格して起動する。
- 起動前に **Safety Rails**（`max_position_size_jpy` / `max_order_value_jpy` / `max_daily_loss_jpy` / `max_orders_per_minute` / `allowed_instruments`、`0` は無効）で発注量・建玉・当日損失・流量に上限をかける。設定 UI と昇格手順は [strategy.md](strategy.md)。 [N5]
- 実行中の run の lifecycle / PnL / ログは **Live Run Panel**（`[Pause]` / `[Resume]` / `[Stop]`）に表示され、Safety Rail 違反は Footer トーストで通知される。 [N1]/[N2]/[N3]/[N4]

## モードによる挙動差

| 項目 | Replay | Manual / Auto（ライブ） |
|---|---|---|
| トランスポートボタン（`|<` `<` `▶/||` `>` `■`） | 表示 [A1]/[A2]/[A3]/[A4] | 非表示 [I4] |
| 再生速度（1x〜50x） | 表示 [A5] | 非表示 [I4] |
| Startup パネル（サイドバー） | 表示 [J7]/[J8] | 非表示 [I4] |
