...
- トランスポート操作・再生速度・Startup ウィンドウは非表示になる（過去データ再生の概念がないため）。フッターのトランスポートボタン可視性は backend から受けた mode 変化と同一フレームで切替わる。 [I4]/[M18]
- **戦略エディタ（Strategy Editor）も非表示**になる。Manual は手動発注のみで戦略編集を使わないため、エディタウィンドウとサイドバーの「Strategy Editor」ボタンを隠す（Replay / Auto では表示）。可視性は backend mode 変化と同一フレームで切替わる。 [M12]/[M19]
