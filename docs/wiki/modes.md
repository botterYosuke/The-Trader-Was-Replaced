...
The Trader Was Replaced は **3 つの実行モード**を持ち、フッター左端のセグメントトグルで切り替える。Bevy 0.15 GUI フロントエンドに Python エンジン（NautilusTrader ベース）を PyO3 で同一プロセスに埋め込んだ（in-proc）単一バイナリ構成。
...
### Manual / Auto ボタンの表示条件

フッターの **Manual** ボタンと **Auto** ボタンは、Venue が `Connected` または `Subscribed` 状態になるまで**非表示**（`Display::None`）になる。`Disconnected` / `Reconnecting` / `Authenticating` / `Error` の間は表示されない。**Replay** ボタンは Venue 状態によらず常に表示される。 [D10]

この表示切替は `apply_venue_live_button_visibility_system` が担い、`VenueStatusRes` の変化と**同一フレーム**で反映される。

### Venue 切断時の自動 Replay 切替

**Manual / Auto モード中に Venue が切断された**場合（`Disconnected` / `Reconnecting` 等）、モードは自動的に **Replay** に切り替わる。 [D11]

- Manual / Auto ボタンが非表示になるのと同一フレームで Replay に戻るため、操作できないモードに留まり続けることがない
- 再度 Venue に接続すれば、手動で Manual / Auto ボタンをクリックして Live モードに戻せる
...
- トランスポート操作・再生速度・Startup ウィンドウは非表示になる（過去データ再生の概念がないため）。フッターのトランスポートボタン可視性は backend から受けた mode 変化と同一フレームで切替わる。 [I4]/[M18]/[M20]
- **戦略エディタ（Strategy Editor）も非表示**になる。Manual は手動発注のみで戦略編集を使わないため、エディタウィンドウとサイドバーの「Strategy Editor」ボタンを隠す（Replay / Auto では表示）。可視性は backend mode 変化と同一フレームで切替わる。 [M12]/[M19]/[M20]

### Auto（ライブ・戦略自動発注）

- **ライブ市場**に接続し、Replay で検証した戦略が**自動で発注**する。
- **Auto への切替自体に `LIVE_VENUE` 配線が要る**。`LIVE_VENUE` 無しではログインできず Auto モードに入れない（＝Auto に入れている時点で venue identity は確定済み）。
...
- 実行中に **▶** を押すと一時停止（`PauseLiveStrategy` 送出、フッター表示 `||`）。一時停止中に再度 **▶** で再開（`ResumeLiveStrategy` 送出、フッター表示 `▶`）。STOPPED / FAILED / Idle のときは新規 run を起動する（`StartLiveAuto`）。 [N13]

## モードによる挙動差

| 項目 | Replay | Manual / Auto（ライブ） |
|---|---|---|
| トランスポートボタン（`|<` `<` `▶/||` `>` `■`） | 表示 [A1]/[A2]/[A3]/[A4] | 非表示（Auto は **▶** のみ表示）[I4]/[N5]/[N13] |
...
| `backend:` | in-proc バックエンドとの接続状態（`DISABLED` / `OK` / `ERR` / `...`） |
