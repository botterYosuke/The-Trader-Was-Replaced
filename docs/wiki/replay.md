...
2. メニューバー **File → Open (Ctrl+O)** で戦略の **サイドカー JSON（`<strategy>.json`）** を選択する（ダイアログは `.json` のみ表示。同名の `.py` が自動で読み込まれ Strategy Editor が開く。JSON の `strategy_path` が別マシンの絶対パス等で存在しない場合は JSON と同ディレクトリの `<stem>.py` へ自動フォールバック）。 [I5]/[I9]/[I19]
