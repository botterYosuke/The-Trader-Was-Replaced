...
1. フッター左端のモードトグルが **Replay** になっていることを確認（既定）。
2. メニューバー **File → Open (Ctrl+O)** で戦略 `.py` を開く（Strategy Editor が開く）。
3. サイドバーの **Startup** パネルで開始日・終了日・粒度・初期資金を確認／編集する。
4. フッター中央の **▶** ボタンで Run を開始する。
5. Replay Startup 進捗ウィンドウが表示され、再生が始まると自動的に消える。
6. トランスポートボタンと速度ボタンで再生を制御する。
...
| ラベル | 内容 | 検証 |
|---|---|---|
| `Start` | 開始日（`YYYY-MM-DD`） | 空・不正な日付はエラー |
| `End` | 終了日（`YYYY-MM-DD`） | 空・不正な日付はエラー。`Start` は `End` 以前であること |
| `Granularity` | 粒度。`Daily` / `Minute` の 2 ボタンから選択 | 未選択・未知の値はエラー |
| `Initial cash` | 初期資金（正の整数） | 整数でない・0 以下はエラー |
...
| ボタン | 動作 |
|---|---|
| `\|<` | 先頭へ（実際には強制停止して初期状態へ戻す） |
| `<` | 1 バック（現状未配線。`PAUSED` 中のみ意味を持つ想定） |
| `▶` / `\|\|` | Play / Pause。`IDLE` / `LOADED` のときは Run（実行開始）、`RUNNING` のときは `\|\|`（Pause）、`PAUSED` のときは `▶`（Resume） |
| `>` | 1 進む（`PAUSED` 中のみ有効。1 バーずつステップ実行） |
| `■` | 強制停止（`RUNNING` / `PAUSED` / `LOADED` のとき有効） |
...
トランスポートボタンの右に速度ボタンが並ぶ。Replay モードのときだけ表示される。
...
Run を開始すると画面中央に **Starting replay** 進捗ウィンドウが表示される。段階ラベルと不確定（左右に往復する）進捗バーを持つ。
...
- **タイムアウト**: 60 秒経っても起動が完了しないと、ソフトタイムアウトのエラーメッセージを表示し、`Close` ボタンが現れる。backend のログを確認するか、`■`（強制停止）を試す。
- **エラー時**: エラーが表示された場合は `Close` ボタンで閉じる。エラーはユーザーが閉じるまで残る。
