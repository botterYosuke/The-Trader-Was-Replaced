...
2. メニューバー **File → Open (Ctrl+O)** で戦略の **サイドカー JSON（`<strategy>.json`）** を選択する（ダイアログは `.json` のみ表示。同名の `.py` が自動で読み込まれ Strategy Editor が開く。JSON の `strategy_path` が別マシンの絶対パス等で存在しない場合は JSON と同ディレクトリの `<stem>.py` へ自動フォールバック）。**すでに Strategy Editor を開いている状態で別の戦略を Open しても、エディタは新しい戦略に差し替わり crash せず、peer/root は 1 個だけになること**（構文ハイライトの構文ツリーは差し替え時に作り直される。二重 spawn による行数チカチカ（#69）を防ぐ）。 [I5]/[I9]/[I19]/[I20]/[I21]
3. **Startup ウィンドウ**で開始日・終了日・粒度・初期資金を確認／編集する。 [J7]/[J8]
...
**Startup ウィンドウ**でシナリオの実行条件を編集する。タイトルバーをドラッグして移動でき、位置・前面化はレイアウト永続化の対象だが [M9]、表示・非表示は常に実行モードが決める [M8]（[ウィンドウとパネル](windows-and-panels.md) を参照）。閉じる `×` ボタンは持たない [M7]。フィールドは戦略のサイドカー JSON（`<strategy>.json` の `scenario` キー）と同期し、編集はキャッシュサイドカーへ書き戻される。
...
> **ウィンドウが無効化される条件**: 再生の起動中（Replay Startup 進捗ウィンドウ表示中）や、書き戻し先のキャッシュサイドカーが未設定のときは、Startup ウィンドウが薄く表示され編集できなくなる。

Startup ウィンドウは **Replay モードのときだけ表示**される（Manual / Auto では非表示）。表示・非表示は実行モードが所有し、レイアウトからは復元されない。 [M8]/[M9]
...
| `>` | 1 進む。`PAUSED` 中は 1 バーずつステップ実行。`IDLE` のときは戦略を flush してから 1 ステップ実行（ForceStop → LoadReplayData → StepReplay。`LOADED` 状態で停止） | [A3] / [A15] |
...
### catalog の precision 互換性（64-bit catalog ↔ high-precision 実行）

nautilus は価格/数量の精度モード（standard 8-byte / high 16-byte）を **コンパイル時** にビルドへ焼き込む。共有 J-Quants catalog は standard-precision（`fixed_size_binary[8]`）で書かれているため、実行側の nautilus も standard でなければならない。high-precision ビルドが standard catalog を読むと、従来は backend が内部で abort し、UI には `transport error` しか出なかった。現在は backend がロード前に幅をチェックし、不一致なら `Catalog precision mismatch ...` という**実原因**を Run Result に表示する（backend は落ちない）。[A12] 対処は [troubleshooting](troubleshooting.md) を参照（`scripts/rebuild_nautilus_standard.sh`）。共有 catalog は書き換えないこと（standard で書く Windows と衝突する）。
...
Startup ウィンドウが編集する開始日・終了日・粒度・初期資金は、戦略 `.py` と**同名の `<strategy>.json`** の `scenario` キーに保存される。SCENARIO の全キー仕様・スキーマバージョン・JSON 例は [strategy.md](strategy.md) を参照。
