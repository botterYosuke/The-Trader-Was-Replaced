...
> **現状（issue #40）**: 専用の **「Promote to Live」ボタン** と **Safety Rails モーダル** は撤去済み。
> UI からの Live Auto 起動入口は一旦無くなり、起動経路はフッターの transport（再生）ボタンへ
> 再配線する予定（フッターで **Auto** に切替 → `[File]▸[Open]` で戦略を読み込み → **▶（再生）** で起動）。
> 起動を駆動する backend の RPC チェーン（`RegisterLiveStrategy` → `SetExecutionMode(LiveAuto)`
> → `StartLiveStrategy`）と Live 実行エンジン・Safety Rails は温存されている（[live-strategy.md](../live-strategy.md) §5）。
