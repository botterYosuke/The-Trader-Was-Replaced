...
| 括弧オートクローズ | 開き括弧（`(` `[` `{` `"` `'`）を入力すると閉じ括弧を補完（直後が閉じ括弧のときは補完しない） [J4] / [J4b] |
| Undo / Redo | `Ctrl+Z`（Undo）/ `Ctrl+Y` または `Ctrl+Shift+Z`（Redo） [I11] |
| 自動保存 | 編集を止めて約 1 秒後にキャッシュへ自動保存（デバウンス）。bevscode の TextBuffer 変更が即時 `StrategyFragment.dirty` を立てる [J1] / [J1b] |

> 空白のみのエディタウィンドウ（中身がスペース・改行のみ）はキャッシュへの書き出しから自動的に除外される。これにより空の `# region` ブロックが `app_state.py` に混入するのを防ぐ。 [J17]

### 検索・置換パネル（`FIND / REPLACE`）

`Ctrl+F` で検索と置換が 1 つのパネルにまとまって開く（置換専用の `Ctrl+H` ショートカットはない）。`Esc` で閉じる。 [J5]

`Ctrl+F` 押下時は bevscode エディタの `InputFocus` を一時的に外し、検索欄への文字入力が同時にコードへも挿入されないようにしている。`Esc` で検索欄を閉じると元のエディタへフォーカスが戻る。Ctrl / Cmd 修飾を伴う文字（`Ctrl+A` 等）は検索欄に流れず、Alt（AltGr 配慮）は通る。 [J5b]
...
## Live Auto への昇格（戦略の自動発注起動）

Replay で検証した戦略を、ライブ venue での**自動発注**として起動する設計になっている。

> **起動経路（issue #40）**: 専用の **「Promote to Live」ボタン** と **Safety Rails モーダル** は撤去され、
> 起動はフッターの transport（再生）**▶** ボタンへ統合された。フッターで **Auto** に切替 → `[File]▸[Open]`
> で戦略を読み込み → **▶** を押すと、scenario に銘柄があり・venue ライブ接続・戦略 cache が揃っていれば
> `RegisterLiveStrategy` → `StartLiveStrategy` を直列発行して run を起動する（いずれか欠けると未送出、
> `SetExecutionMode` は ▶ から再送しない＝モードは backend 権威）。起動を駆動する backend の RPC チェーン
> （`RegisterLiveStrategy` → `StartLiveStrategy`）と Live 実行エンジン・Safety Rails は温存されている
> （[live-strategy.md](../live-strategy.md) §5）。起動銘柄は scenario（サイドカー JSON）から導出する（Replay Run と対称。複数銘柄ではサイドバー選択が scenario 内ならそれを、無ければ先頭）。前提（instrument・venue ライブ接続・strategy ロード）が欠けて未送出になった場合、▶ はサイレントに終わらず **Run Result パネル**に理由を赤字表示する（"No instrument selected" / "Venue not connected" / "No strategy loaded (open a strategy file)"）。 [N5]/[N6]/[N7]

起動後の run の lifecycle・PnL / order / fill テレメトリは **Run Result** パネルにリアルタイム表示される。戦略ログ tail は `StrategyLogs` リソース（リングバッファ 100 行）に保持される。詳細は [orders.md](orders.md) / [windows-and-panels.md](windows-and-panels.md)。 [N1]/[N2]/[N4]

## Safety Rails（ライブ自動発注の安全装置）

Live Auto の暴走を防ぐため、起動時に発注の上限を `StartLiveStrategy` へ渡す。**`0` はそのレールを無効**にする（課さない）。上限は数値 4 項目（既定値: max_position_size_jpy=1,000,000 / max_order_value_jpy=500,000 / max_daily_loss_jpy=100,000 / max_orders_per_minute=5）と `allowed_instruments` のホワイトリストからなり、バックエンドが pre/post-trade で強制する。
