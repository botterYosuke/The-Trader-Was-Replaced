...
backend が auto-restart した後、UI が working と信じている注文のうち**再起動後の backend が追跡していない**ものだけが ReconcileModal に表示されます（terminal orders は無視）。 [K6] 通知の dismiss / Escape 優先順位は [K14]。Escape での dismiss は **#46 Slice B でモーダルスタック（`ModalLayer`）に一本化**されました：従来は ReconcileModal 専用の Esc system が個別に消していましたが、現在は汎用の `modal_layer_esc_system` が frontmost のモーダルを dismiss し、secret / 発注確認 / 訂正モーダルが開いているときはそちらに Esc を譲ります（観測可能な優先順位は従来どおり [K14]）。モーダルは通知のみで、再ログインと注文の再確認は venue 側で行います。同じ `GetOrders` 応答から、backend が追跡している注文は OrdersPanel に完全な注文行として seed されます（id-diff の reconcile と完全行の seed を同じ snapshot から行う） [H9]。
