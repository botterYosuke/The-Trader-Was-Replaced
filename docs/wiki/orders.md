...
- EC stream で ACCEPTED / PARTIALLY_FILLED / FILLED / CANCELED を受けると、30s ポーリングを待たずに即座に `account_sync.force_resync()` が走り、Buying Power / Positions が更新されます [H13]。REJECTED / SUBMITTED 等の non-state-changing ステータスはスキップしオーバーフェッチを防ぎます。
- `AccountSync` は VenueLogin 後から Replay 遷移（Live session 終了）まで 30s 間隔で実行し続けます。LiveManual→LiveAuto 等の Live 内モード遷移では停止されません [H15]。同一スナップショットが続く場合は re-emit をスキップし（差分 push）、変化があったときだけ AccountEvent を送ります [H15]。
...
backend が auto-restart した後、UI が working と信じている注文のうち**再起動後の backend が追跡していない**ものだけが ReconcileModal に表示されます（terminal orders は無視）。 [K6] 通知の dismiss / Escape 優先順位は [K14]。Escape での dismiss は **#46 Slice B でモーダルスタック（`ModalLayer`）に一本化**されました：従来は各モーダル専用の Esc system が個別に消していましたが、現在は 5 つのモーダル（secret / 発注確認 / 訂正 / reconcile / relogin）すべてが `ModalLayer` スタックに載り、汎用の `modal_layer_esc_system` が単一 Escape で **dismiss 優先度 `z` が最高のモーダル**を閉じます（secret 300 > confirm 280 > modify 270 > reconcile 262 > relogin 260）。secret や発注確認が開いていれば先にそれらが閉じ、ReconcileModal は survive します——観測可能な優先順位は従来どおり [K14]。モーダルは通知のみで、再ログインと注文の再確認は venue 側で行います。同じ `GetOrders` 応答から、backend が追跡している注文は OrdersPanel に完全な注文行として seed されます（id-diff の reconcile と完全行の seed を同じ snapshot から行う） [H9]。

## Auto モードの発注（Live Auto 戦略実行）

**Auto** モードでは、起動した戦略がライブ venue に**自動で発注**します。起動経路（現状の入口と Safety Rails）は [strategy.md](strategy.md) を、モード概要は [modes.md](modes.md) を参照してください。実行中の run の状態・PnL・order/fill は **Run Result** パネルに表示されます（[windows-and-panels.md](windows-and-panels.md) 参照）。[N1]/[N2]

## バックエンドイベント

in-proc で埋め込まれた Python エンジンから UI へ向けて、内部チャネル（`SubscribeBackendEvents`）を通じて `BackendEvent` が push されます（`src/trading.rs`）。旧 gRPC のサーバー送信ストリームは #64 / #68 で in-proc チャネルに置き換えられました。
...
| LiveStrategyEvent | Live Auto run の lifecycle 変化（READY / RUNNING / PAUSED / STOPPING / STOPPED / ERROR） | `CurrentRun` | [N1] |
| LiveStrategyTelemetry | run-scoped の PnL / order / fill カウンタ（任意の頻度で届く） | `CurrentRun` | [N2] |
| SafetyRailViolation | pre/post-trade の Safety Rail 違反 | `SafetyToast` | [N3] |
| StrategyLogMessage | 戦略の `self.log.*` 行の中継（StrategyLogs リングバッファ） | `StrategyLogs` | [N4] |

ユーザー視点では、SecretRequired を受けると第二暗証番号モーダルが表示され、OrderEvent / AccountEvent によって Orders / Positions / Buying Power が更新され、VenueLogoutDetected を受けると ReloginModal が開いて再ログインを促します（モーダルは通知のみで、再ログイン自体は Venue メニューから行います。Escape での dismiss は #46 Slice B で `ModalLayer` 経由の汎用 `modal_layer_esc_system` に統一済み — 優先順位は [K13]）。Live Auto では LiveStrategyEvent / LiveStrategyTelemetry / StrategyLogMessage が `CurrentRun` / `StrategyLogs` を更新して Run Result パネルに反映し、SafetyRailViolation は Footer トーストで通知されます。
