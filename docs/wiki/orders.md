...
- **注文 → 約定 → ポートフォリオ反映パイプライン**: 発注した注文の状態（受付・約定・拒否など）と約定結果が口座に反映されます。発注成功時は full レコードが `LiveOrders` に seed され [H1]、以後のステータス/約定更新がマージされます [H2]/[F3]。訂正は qty/price の差分のみ上書きされ [H3]、拒否は OrderPanel のエラー行に整形メッセージが出ます [H4]。実行モードを切り替えると口座スナップショットは一旦リセットされ、Live/Replay のデータが混ざらないようになっています [H5]。
...
| VenueLogoutDetected | venue 側でのログアウト検知 | [D5]（保留中） |

> SecretRequired / OrderEvent / AccountEvent の 3 つは Phase 9 マージで `backend_event_drain_system` に reducer が入り、それぞれ `SecretPrompt` / `LiveOrders` / `PortfolioState` を更新するため E2E で観測可能になりました（[F5]/[F3]/[F4]）。VenueLogoutDetected はまだ `info!` のみで resource を変えないため [D5] は保留中です（assert 可能にするには `VenueStatusRes` を切断側へクリアする本番拡張が必要）。

ユーザー視点では、SecretRequired を受けると第二暗証番号モーダルが表示され、OrderEvent / AccountEvent によって Orders / Positions / Buying Power が更新され、VenueLogoutDetected を受けると venue 状態バッジが切断側に戻ります。
