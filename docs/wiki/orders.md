...
- **口座同期**: venue 側の現金残高・建玉を取得して Positions / Buying Power に反映します。
...
| イベント種別 | 意味 | E2E |
|---|---|---|
| SecretRequired | 第二暗証番号の入力要求（Tachibana のみ。kabu は送出しない） | [F5]（保留中） |
| OrderEvent | 注文状態の更新（ステータス・約定数量・平均約定価格など） | [F3]（保留中） |
| AccountEvent | 口座更新（現金・買付余力・建玉一覧） | [F4]（保留中） |
| VenueLogoutDetected | venue 側でのログアウト検知 | [D5]（保留中） |

> 上記 4 つの event seam は `backend_event_drain_system` が resource を更新しないため現状 assert できず、`FLOWS.md` で保留扱いです。Phase 9 で実装したら flow を有効化します。

ユーザー視点では、SecretRequired を受けると第二暗証番号モーダルが表示され、OrderEvent / AccountEvent によって Orders / Positions / Buying Power が更新され、VenueLogoutDetected を受けると venue 状態バッジが切断側に戻ります（いずれも Phase 9 で開発中）。
