...
**Auto** モードでは、起動した戦略がライブ venue に**自動で発注**します。起動経路（現状の入口と Safety Rails）は [strategy.md](strategy.md) を、モード概要は [modes.md](modes.md) を参照してください。実行中の run の状態・PnL・ログは **Live Run Panel** に表示されます（[windows-and-panels.md](windows-and-panels.md) 参照）。
