...
- **M 群（window / sidebar）✅** — m1 sidebar Panels ボタン spawn, m2 drag→位置更新+autosave dirty, m3 close→despawn, m4 focus→z 前面, m5 duplicate policy（StrategyEditor のみ multi）, m7 Startup ウィンドウは × を持たない（closeable:false 回帰ガード）, m8 Startup ウィンドウ可視性は ExecutionMode が所有（Replay のみ可視）, m9 Startup（screen-space `ScreenWindowRoot`）位置(Node left/top)/z は WindowLayout で復元・size と `visible` は非権威（ADR 0003）, m10 resize handle Drag→幅拡大+最小サイズクランプ+DragEnd→autosave dirty（`resizable:true` 回帰ガード）, m11 Startup を Manual/Auto で隠すと枠だけでなく中身（Start/End/Granularity/Initial cash）も隠れる＝`content_area` が `Visibility` を持ち可視性伝播が中身まで届く（root→content 経路の全 entity が `InheritedVisibility` を持つことを assert する回帰ガード。m8 は root の `Visibility` しか見ず中身残存バグを取りこぼした）, m12 Strategy Editor（フローティング窓＋サイドバーボタン）は `ExecutionMode::LiveManual` のときだけ隠れる（Replay / LiveAuto は表示）。save/restore で layout の `visible` 権威を壊さず、Manual 中に spawn された窓も追従して隠す（kind:ui）。実 Strategy Editor を spawn し editor(`TextInputNode`=`StrategyEditorContent`)→content_area→root の `Visibility`+`InheritedVisibility` 連鎖を構造 assert する＝枠だけでなく中身も隠れる（screen-space Bevy UI 化 ADR 0003 で gutter/scrollbar は撤去。m11 同型・root の `Visibility` しか見ずに中身残存バグを取りこぼす経路を防ぐ回帰ガード）, m13 screen-space window（Strategy Editor / Startup・`ScreenWindowRoot`）の position(Node left/top)/size(width/height) が Save で sidecar JSON に永続化される（`build_layout` の screen 分岐・ADR 0003 follow-up を close。A-2 では Node window が `&Sprite` クエリに掛からず geometry が黙って失われていた）, m14 既存 screen-space Strategy Editor window を `apply_layout_system` が復元（pos/size/visible すべて layout 権威。world-space match 空振り後に `Without<ScreenWindowRoot>`/`With<ScreenWindowRoot>` で分離した screen クエリで match し `Node` geometry を書く）。`m6`（settings sidebar）は固定文字列で BackendStatus 未接続 → doc stub。
...
| レイアウト永続化 | ファイル I/O と debounce が主対象。world-space sprite window と screen-space `Node` window（ADR 0003）の 2 種を 1 つの `windows` 配列で save/restore する | temp dir fixture で `Save/Load` を integration 実行し JSON と復元 entity を assert。screen window は `Node` left/top/width/height + `GlobalZIndex` を round-trip | I7 ✅ / I12 ✅ / M2 ✅（drag autosave） / M13 ✅（screen window save） / M14 ✅（screen editor restore） |
| text editor 入力 | text editor の focus/keyboard 処理が主対象 | `kind:ui`。focused entity と keyboard/text input を注入。必要なら最小実ウィンドウ smoke を追加 | J1 ✅（J2-J4=Tab/autoindent/bracket は ADR 0003 で撤去） |
| ~~find / replace~~ | — | ADR 0003 で撤去（`bevy_ui_text_input` に検索 UI なし） | J5/J6 撤去 |
...
| Startup ウィンドウ（× 無し / mode 所有 / 位置永続化） | screen-space floating window（`spawn_screen_window`、フィールドは `TextInputNode`・ADR 0003）だが close 不可・可視性は ExecutionMode 所有・layout の `visible` 非権威（ADR-0001）。位置は `Node` left/top として sidecar に永続化（size は窓側定数が正で復元しない） | `kind:ui`。本番 spawn の `CloseButton` 不在、`apply_startup_panel_visibility_system` の `Visibility`、root→content の可視性伝播が `content_area` の `Visibility` で繋がること（中身も隠れる）、`apply_layout_system` 復元時の Node left/top 復元と size/visible 無視を assert | M7 ✅ / M8 ✅ / M9 ✅ / M11 ✅ / M13 ✅ |
