...
- **N 群（Live Auto strategy execution、Phase 10）✅** — n1 LiveStrategyEvent→`CurrentRun` lifecycle（run_id/strategy_name/started_ts_ms 固定・空 strategy_id は既知を消さない・state: RunState）, n2 LiveStrategyTelemetry→`CurrentRun` カウンタ（lifecycle 前に届いても counters 更新 / run_id は None のまま / lifecycle が telemetry を消さない）, n3 SafetyRailViolation→`SafetyToast`（最新が古いトーストを置換）, n4 StrategyLogMessage→`StrategyLogs`（oldest-first リング・`recent(n)`・`CAP`=100 で頭切り）。全 `kind:state`、既存 `Harness` の `current_run()` / `safety_toast()` / `strategy_logs()` で観測。（`LiveRuns` と `live_run_panel.rs` は issue #42 Slice 2 で撤去済み。）（旧 n5 LiveStrategyPromoteResult→`PromoteFeedback` は Promote to Live 入口の撤去に伴い削除＝issue #40。Live 実行エンジンと n1–n4 は温存。） n5（id 再採番・**kind:ui**、n1–n4 の kind:state とは別）footer ▶ で Live Auto 起動: `ExecutionMode::LiveAuto` で scenario instruments・venue ライブ・venue identity(`venue_id` か `configured_venue`)・cache_path が揃うと `StartLiveAuto{instrument_id,venue,strategy_file}` を 1 件送出（起動銘柄は scenario 由来＝Replay と対称、複数は選択優先/先頭。`SetExecutionMode` は再送しない）、いずれか欠けると未送出。`Harness::click_pause_resume()` で実 UI 駆動・`drain_commands()` で観測。n6（**kind:ui**）は本番 `spawn_footer` の実 ▶ entity を production system で駆動し「entity 生成→可視性→押下→送出」の実機リンクを検証（real-footer integration。N5 は command-level の合成 entity）。footer visibility は footer.rs unit（PauseResume は Replay/LiveAuto で表示、他 transport/speed は Replay のみ）で担保。
