...
- **I 群（メニュー / file / layout）✅** — i1 メニュー開閉, i2 Alt+F/E/V, i4 モード切替 gating（未送信 assert）, i5 file-open→Editor/Chart spawn, i6 File→New, i7 Save→sidecar, i9 Ctrl+S/Shift+S/O dispatch, i10 open→Auto, i11 Edit→Undo/Redo, i12 起動時 cache 復元, i13 scenario-only JSON。`i3`（Escape/outside-close）は本番未実装のため doc stub。`i8`（Save As）と `i14`（path 未確定 Save → Save As 相当ダイアログにフォールバック）は rfd ダイアログ直呼びで headless 不可 → `#[test] #[ignore]`（同 seam を分割するリファクタで両方テスト可能になる）。**`i15`（cache 復元直後に Replay 突入しても `app_state.py` が scenario `.json` で上書きされない）は現在 RED＝既知バグの回帰ガード**: `restore_fixed_registry_on_replay_entry_system` が `ScenarioReadTarget`(=`.json`) を `.py` 期待の `StrategyFileLoadRequested` に流し、`sync_to_cache` が `app_state.py` を JSON で上書きする（`editable=false`/instruments_ref ロック時）。fix（restore.rs の経路是正＋`sync_to_cache` の `.py` ガード）後に green になる（#23）。
...
| Startup ウィンドウ（× 無し / mode 所有 / 位置永続化） | sprite floating window だが close 不可・可視性は ExecutionMode 所有・layout の `visible` 非権威（ADR-0001） | `kind:ui`。本番 spawn の `CloseButton` 不在、`apply_startup_panel_visibility_system` の `Visibility`、`apply_layout_system` 復元時の pos/z と visible 無視を assert | M7 ✅ / M8 ✅ / M9 ✅ |
