...
- **N 群（Live Auto strategy execution、Phase 10）✅** — n1 LiveStrategyEvent→`LiveRuns` lifecycle（upsert・started_ts_ms 固定・空 strategy_id は既知を消さない）, n2 LiveStrategyTelemetry→`LiveRuns` カウンタ（lifecycle 前に届いても row 作成 / lifecycle が telemetry を消さない）, n3 SafetyRailViolation→`SafetyToast`（最新が古いトーストを置換）, n4 StrategyLogMessage→`StrategyLogs`（oldest-first リング・`recent(n)`・`CAP`=100 で頭切り）。全 `kind:state`、既存 `Harness` の `live_runs()` / `safety_toast()` / `strategy_logs()` で観測。（旧 n5 LiveStrategyPromoteResult→`PromoteFeedback` は Promote to Live 入口の撤去に伴い削除＝issue #40。Live 実行エンジンと n1–n4 は温存。）

残る doc stub（headless 不可 / 未実装の release gate）: `i3`(menu Escape 未実装), `k1`/`l4`(kind:render), `l1`(Windows PowerShell), `m6`(未接続)。Live Run Panel の **UI 操作**（`[Pause]`/`[Resume]`/`[Stop]` クリック→`TransportCommand`）は `kind:ui` で未実装（N 群は backend→ECS の state seam のみ）。Promote to Live 入口（ボタン + Safety Rails modal）は issue #40 で撤去済み。LiveAuto の起動はフッター transport（再生）ボタンへ再配線予定。
