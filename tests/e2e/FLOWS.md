...
残る doc stub（headless 不可 / 未実装の release gate）: `i3`(menu Escape 未実装), `k1`/`l4`(kind:render), `l1`(Windows PowerShell), `m6`(未接続)。Live Run Panel は issue #42 Slice 2 で撤去済み。Promote to Live 入口（ボタン + Safety Rails modal）は issue #40 で撤去済み。LiveAuto の起動・Pause/Resume はフッター **▶** ボタンへ統合済み [N5]/[N13]（起動: `StartLiveAuto`; Running → `PauseLiveStrategy`; Paused → `ResumeLiveStrategy`）。
