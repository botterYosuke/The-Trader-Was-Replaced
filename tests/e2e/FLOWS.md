...
残る doc stub（headless 不可 / 未実装の release gate）: `i3`(menu Escape 未実装), `k1`/`l4`(kind:render), `l1`(Windows PowerShell), `m6`(未接続)。Live Run Panel の **UI 操作**（`[Pause]`/`[Resume]`/`[Stop]` クリック→`TransportCommand`）は `kind:ui` で未実装（N 群は backend→ECS の state seam のみ）。Promote to Live 入口（ボタン + Safety Rails modal）は issue #40 で撤去済み。LiveAuto の起動はフッター transport（再生）**▶** ボタンへ再配線済み [N5]（▶ クリックで `StartLiveAuto`→`RegisterLiveStrategy`/`StartLiveStrategy` 直列起動）。
...
| window 操作 | drag/close/focus/duplicate が observer + ECS | `kind:ui`。observer trigger と Transform/WindowManager/AutoSaveState を assert | M1-M5 ✅ / M6 stub(未接続) / M10 ✅（resize handle） / M21 ✅（Pickable） |
