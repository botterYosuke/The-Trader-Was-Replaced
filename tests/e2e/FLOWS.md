...
| footer `apply_execution_mode_visibility_system` が backend mode 変化に同一フレームで追従 | `apply_execution_mode_visibility_system` は `is_changed()` ガード付きだが `.after(status_update_system)` 制約が無く、同フレームで前に走ると 1 フレーム遅延する（issue #41 codex review の Medium。footer.rs:809）。⚠️ production コードに `.after(status_update_system)` を追加することで fix | `kind:state`。Harness で実 `status_update_system` 経由に `ExecutionModeChanged` を流し、`apply_execution_mode_visibility_system.after(status_update_system)` の下で `PauseResumeButton Node.display` が mode 確定と同一 tick で反映されることを assert | M18 ✅ |
| strategy editor `apply_strategy_editor_mode_visibility_system` が backend mode 変化に同一フレームで追従 | `apply_strategy_editor_mode_visibility_system` は `is_changed()` ガード無しで毎フレーム読むが `.after(status_update_system)` 制約が無く、同フレームで前に走ると古い mode で判断する（issue #41 codex review の Medium。strategy_editor.rs:172）。⚠️ production コードに `.after(status_update_system)` を追加することで fix | `kind:state`。Harness で実 `status_update_system` 経由に `ExecutionModeChanged{LiveManual}` を流し、`apply_strategy_editor_mode_visibility_system.after(status_update_system)` の下で `PanelKind::StrategyEditor` entity `Visibility` が mode 確定と同一 tick で Hidden になることを assert | M19 ✅ |
