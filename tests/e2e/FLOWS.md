...
- [x] **N8（kind:state）gRPC reject → `CurrentRun.Failed` 回帰ガード** — `RunFailed{startup_id:None, error:"RegisterLiveStrategy rejected:..."}` を `apply_status_update` 直接注入し `CurrentRun.state` が `RunState::Failed{error}` になることを確認（#42 Slice 2 の「main.rs StartLiveAuto ハンドラで gRPC reject → RunFailed 送出」実装の回帰ガード）。
