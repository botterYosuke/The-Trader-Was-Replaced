...
/// Multiplier applied to the render texture resolution without changing the logical layout.
///
/// When > 1.0, `render.rs` rasterizes a cloned buffer with scaled Metrics into
/// a larger pixel buffer, then writes that to the sprite's texture. The logical
/// `Sprite.custom_size` and `Transform` remain unchanged, so hit-testing and
/// text wrapping are not affected.
#[derive(Component, Clone, Debug)]
pub struct CosmicRenderScale(pub f32);

impl Default for CosmicRenderScale {
    fn default() -> Self {
        Self(1.0)
    }
}
