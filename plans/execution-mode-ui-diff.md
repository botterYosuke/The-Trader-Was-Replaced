...
ガード対象となる隙間:
- mode 切替が反映された直後のフレームで、前フレームに発火していた `Interaction::Pressed` が
  まだ残っている stale 状態
...
いずれも `exec_mode: Res<ExecutionModeRes>` を追加し、関数頭で
...
`footer_pause_resume_system` と `speed_button_system` も同じパターン。

理由: `Display::None` で `Interaction` 経路は塞がるが、stale Pressed・テスト・将来の
programmatic interaction に対する安全策。**早期 return は backend へ送る command 経路
全てを 1 箇所で塞ぐ**ので、追加コストは function 頭の 3 行のみ。
...
| `pause_resume_does_not_emit_run_event_in_manual` / `_in_auto` | Run flow (footer.rs:586-613) は `replay_state` が `RUNNING`/`PAUSED` 以外 **かつ** `cache_path` が `Some` のときだけ event を emit | `data.replay_state = Some("IDLE".into())` + `buffer.cache_path = Some(tempfile_path)` |
...
PauseResume 系の cache_path には `tempfile::tempdir()` で test スコープに閉じた一時ファイルを
渡す（gate 無しでは `flush_strategy_cache` がディスクに書き込むため、test の test-isolation 衛生上
プロジェクトルートやホームディレクトリを汚さない）。gate ありの GREEN-1 時には early-return で
ファイルに触れないので tempfile が空のままで OK。
...
13 テストを一括追加すると、step 2 時点で `apply_execution_mode_visibility_system` 未定義
のためコンパイル不能 → `cargo test` 全体が走らない。**RED→GREEN を「gate 系」と
「visibility 系」で 2 回に分割する**:
...
4. **GREEN-2 (visibility)**: `footer.rs` に `apply_execution_mode_visibility_system` を実装。
   可視性 7 テスト green。
5. **REGISTER**: `mod.rs` 末尾に独立 `add_systems(Update, (...))` ブロックを 1 つ追加し
   visibility 2 system を登録（オプション B フラット構成）。既存ブロック (mod.rs:152-176,
   現 18 system) を膨らませずタプル 20 上限のマージンを残す。
6. **RED-3 (startup panel)**: `scenario_startup_panel.rs::tests` に 4 テスト追加。
7. **GREEN-3 (startup panel)**: `apply_startup_panel_visibility_system` を追加、step 5 の
   ブロックに登録。
...
- **変更すること**:
  - `transport_button_system` / `footer_pause_resume_system` / `speed_button_system` の
    シグネチャに `exec_mode: Res<ExecutionModeRes>` を追加し、Replay 以外で早期 return。
