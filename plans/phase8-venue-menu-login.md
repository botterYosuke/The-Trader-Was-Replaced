...
テスト (`python/tests/exchanges/test_tachibana.py`, `test_kabusapi.py` 既存ファイルに追加):
- `is_logged_in` が `False`/`True` を正しく返すこと (session/token を直接代入してチェック)。
- Tachibana session_cache 経路: `save_session` で書いた dict を `login()` で復元できる (`tachibana_file_store` を temp dir で monkeypatch)。
...
- `tachibana_login_flow.py` — tkinter view + asyncio ブリッジ。
  ...
  - **async ブリッジ**: OK callback は同期だが `engine.exchanges.tachibana_auth.login(...)` は async。callback では:
    ...
    2. `result_holder: dict = {}` を closure capture して **新規スレッド** で `result_holder["v"] = asyncio.run(tachibana_auth.login(...))` を回し、終了後に `root.after(0, lambda: _on_auth_done(...))` で UI スレッドへ復帰。`asyncio.run` を直接 callback で呼ぶと `httpx.AsyncClient` の close warning + tk mainloop 停止のリスクがあるためスレッド分離する。
    3. `_on_auth_done` 内で `save_session(...)` (成功時) / error code セット (失敗時) → `root.destroy()`。
- OK 成功時 save_session 入力 dict:
  ...
  仮想 URL のみ。ID/PW はディスクに書かない (skill S3)。
...
async def _handle_prompt_login(self, venue_id: str, env_hint: str) -> tuple[bool, str, Optional[str]]:
    ...
    try:
        ...
        if not line:
            ...
            logging.error("login_dialog_runner exited without result: %s", stderr)
            await proc.wait()
...
def VenueLogin(self, request, context):
    # ... (token / venue_id / configured_venue / idempotent チェックまでは既存維持) ...
        ...
        try:
            ...
            if effective_source == "prompt":
                fut = asyncio.run_coroutine_threadsafe(
                    self._handle_prompt_login(venue_id, env_hint or ""),
                    loop,
                )
...
- `is_venue_busy_for_menu(status.state)` が true、かつ `status.venue_id.as_deref()` が押下対象の反対 venue (例: `MenuItem::VenueConnectKabu*` を判定する時に `status.venue_id == Some("TACHIBANA")`) なら、ボタンを灰色 (`Color::srgba(0.20, 0.20, 0.20, 0.5)`) + `Text` の `TextColor` を暗くする。同 venue 側と Disconnect は通常色。
...
uv run pytest \
    python/tests/live/ \
    python/tests/exchanges/test_tachibana.py \
    python/tests/exchanges/test_kabusapi.py \
...
- `python/tests/exchanges/test_tachibana.py`, `test_kabusapi.py` (is_logged_in / session_cache / prompt_result テスト追加)
...
- **Lane C (Python wiring)**: Step 5, 6 (server_grpc subprocess IPC + factory)。tdd-workflow skill 準拠。Lane A/B 完了後に着手 (Step 5 のテストが Lane A/B の API に依存)。
