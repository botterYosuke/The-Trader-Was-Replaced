...
- 判定: 閉局（`sSystemStatus != "1"`）または ログイン不許可（`sLoginKyokaKubun` not in `{"1","9"}`）を
  「本体ログアウト → 要再ログイン」とみなす。SS は**接続毎に初回再送**されるため、open→closed の
  **遷移時のみ 1 回**通知する（debounce。`_last_system_open` で管理、login/logout でリセット）。
