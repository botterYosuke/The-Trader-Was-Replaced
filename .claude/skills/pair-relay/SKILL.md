...
これらは全部 Navigator（または新規 Navigator）の仕事。司令塔がやると:
1. Navigator のレビュー責務が抜けて、検証なしの「たぶんこれ」修正がそのまま走る
2. 司令塔の context が原因調査ログで膨らみ、本来の監視・swap 判断ができなくなる
...
- **現 Navigator が生きている**: User の言葉をそのまま現 Navigator に転送する。
  例: 「User からレビュー: 『X すると Y が起きる』。原因調査と修正をお願いします。Navigator 内ループ (N1)〜(N5) を回してから完了報告して」
- **現 Navigator がもう context を使い切っている / 既に完了報告を返して終わっている**: 新 Navigator を spawn する。spawn prompt に「最初のタスクは User から来たレビュー対応: <User の言葉そのまま>。原因調査も Navigator の責務」と書く。
- **User の報告が曖昧で再現条件が分からない**: 司令塔が User に再現手順を 1 回だけ聞き返す。コードは開かない。

司令塔は「User の言葉を Navigator に運ぶ郵便配達」に徹する。原因究明の入口を司令塔が踏まない。

例外: User が「司令塔（あなた）が直接見て」「Navigator 通さず読んで」と明示したときのみ司令塔が `Read`/`Grep` を使ってよい。それでも Edit は Driver（Navigator 経由）に任せる。
...
| User からのレビュー・バグ報告を見て司令塔が `Read`/`Grep` で原因調査を始める | 調査は Navigator の責務。司令塔は User の言葉をそのまま Navigator に転送するだけ（「User からレビュー・修正依頼が返ってきたとき」節参照） |
| 司令塔が User の報告から原因を推測して Navigator に「ここを直して」と修正方針付きで指示する | 調査と修正方針決定は Navigator の (N1) でやる仕事。司令塔は症状だけ転送し、原因究明と diff 作成は Navigator に任せる |
