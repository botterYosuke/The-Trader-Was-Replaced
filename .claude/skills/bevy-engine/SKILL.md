...
**Startup 自動 spawn パターン（mode-owned パネル）**: sidebar ボタンを持たず起動時に自動 spawn するパネル
（STARTUP / RUN RESULT が実例）では、ヘルパーとは別に **Bevy system ラッパーを用意** する必要がある。

```rust
// ヘルパー関数 — dispatcher / test から `&mut commands` で呼ぶ
pub fn spawn_xxx_panel(commands: &mut Commands) { ... }

// Bevy system ラッパー — add_systems(Startup, ...) に直接登録できる形式
pub fn spawn_xxx_panel_system(mut commands: Commands) {
    spawn_xxx_panel(&mut commands);
}
```

`add_systems(Startup, spawn_xxx_panel)` に `fn(commands: &mut Commands)` を渡すと
`IntoSystemConfigs` が実装されておらず **E0277** でコンパイルエラーになる
（`&mut Commands` は `SystemParam` ではなく、Bevy システムタプルに入れられない）。
`spawn_scenario_startup_window_system` が実例（issue #41 で RunResult が同じ罠を踏んだ）。
