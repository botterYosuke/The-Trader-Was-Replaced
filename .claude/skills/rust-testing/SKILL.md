...
- **Port-reuse flakiness (`WSAEADDRINUSE`) → `serial_test` で直列化.** 固定ポートを使う統合テスト群は `cargo test` の並列実行で同ポートを奪い合い `WSAEADDRINUSE` で断続的に落ちる（特に Windows）。根本解決は `serial_test` クレートを `[dev-dependencies]` に追加し、`use serial_test::serial;` を宣言した上で全テストに `#[serial]` を付与すること。ポートをユニークにするだけでは並列実行時の競合を防げない。`serial_test` は OS ポート解放の待機も不要にし、CI の `--test-threads=N` 設定に依存しない安定性をもたらす。
