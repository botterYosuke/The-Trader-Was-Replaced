...
| 症状 | 原因 | 対処 |
|---|---|---|
| `backcast.exe` が **起動直後にログ 0 バイトで即終了**（`Start-Process ... -PassThru` の `ExitCode=-1073741515` / `0xC0000135` = `STATUS_DLL_NOT_FOUND`） | issue #64 の PyO3 in-proc embedding 以降、exe が **`python3.dll` をハードリンク**する。PATH に python DLL ディレクトリが無いと OS ローダ段階（Rust main 到達前）で落ち、tracing ログも一切出ない（GH #66） | 起動 env に **python DLL ディレクトリを PATH 追加**する。uv 管理 python なら `sys.base_prefix`（例 `C:\Users\<u>\AppData\Roaming\uv\python\cpython-3.14.x-windows-x86_64-none`）に `python3.dll`/`python3xx.dll` がある（venv の `Scripts` には無い）。`$env:PATH = "$pydir;$env:PATH"` を §2.3 の env 注入に足す。⚠️ **`cargo test --lib` が緑でも GUI 起動はこの DLL trap で別に落ちる**（test exe は cargo の build 環境で DLL が解決されるため）。E2E では必ず実 exe を起動して確認する |
| `backcast.exe` 起動直後に panic `there is no reactor running, must be called from the context of a Tokio 1.x runtime`（`backend_transport.rs`、system `setup_backend_connection`） | transport の `run()` が返り値 future の外（同期ボディ）で `tokio::spawn` していると、`main.rs` の `handle.spawn(transport.run(...))` が `run()` をランタイム外（Bevy compute pool）で同期評価して落ちる | `tokio::spawn` を `Box::pin(async move { ... })` の**内側**へ移す。**この回帰は `cargo test --lib` では出ない**（unit test は実 Bevy ランタイム経路 `setup_backend_connection` を通らない）。GH #64 で修正済み・回帰ガード `grpc_run_does_not_spawn_eagerly_outside_runtime` |
| File→Open でサイドカー JSON を開いても Strategy Editor が空（"dummy"/untitled）・Startup の日付が反映されない | JSON 内 `strategy_path`（絶対パス）が現マシンに無いと `apply_layout_system` が `.py` ロードを skip し、sibling `<stem>.py` にフォールバックしない（GH #67）。または起動時に壊れた cache `app_state.json`（`end:"not-a-date"` 等）が復元され scenario に焼かれている | ログを `menu: load layout requested` → `scenario-only JSON ... loading sibling strategy` → `strategy file load requested` → `SCENARIO parsed from sidecar: ... end=...` の順で grep し、どの段で止まったか確認。`WARN layout load: strategy_path ... not found, skipping` が出れば #67。cache 汚染なら `C:\Users\<u>\AppData\Local\the-trader-was-replaced\app_state.json` を退避して再起動 |
