...
Thin façade over GrpcDataEngineServer that lets Rust call live Python methods
directly via PyO3, bypassing gRPC TCP+protobuf round-trips.

Design decisions:
- GrpcDataEngineServer is instantiated with token="" so _token_ok() always
  passes (hmac.compare_digest("", "") == True).
- _NullContext.abort() raises RuntimeError; each public method wraps its call
  in try/except and returns {"success": False, "error_code": "INPROC_ABORT"}.
- All return values are plain Python dicts so PyO3 can extract them without
  proto imports on the Rust side.
- get_state_json() delegates to GrpcDataEngineServer.GetState() so live
...
class _NullContext:
    ...
    def abort(self, code, details):
        raise RuntimeError(f"[inproc] abort {code}: {details}")
...
class InprocLiveServer:
    """Direct-call façade over GrpcDataEngineServer for in-process Rust dispatch."""
    ...
    def get_state_json(self) -> str:
        """Return JSON from GrpcDataEngineServer.GetState() (includes live prices/depth)."""
        ...
        try:
            ...
            logging.exception("[inproc] get_state_json failed; falling back to DataEngine")
    ...
    def start_engine(self, cfg: dict) -> dict:
        ...
        if cfg.get("granularity"):
            from .server_grpc import GrpcDataEngineServer as _S
            gran = _S._replay_granularity_name(None, _parse_granularity_int(cfg["granularity"]))
            if gran:
                engine_start_config.granularity = _parse_granularity_int(cfg["granularity"])
